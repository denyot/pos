...
public class RequestFilter implements Filter {
	...
	public void doFilter(ServletRequest pRequest, ServletResponse pResponse,
		...
		postType = G4Utils.isEmpty(postType) ? "0" : postType;
		if (postType.equals("1")) {
			long start = System.currentTimeMillis();
			fc.doFilter(request, response);
			if (eventMonitorEnabel.equalsIgnoreCase("1")) {
				costTime = new BigDecimal(System.currentTimeMillis() - start);
				saveEvent(request, costTime);
			}
		} else {
			String isAjax = request.getHeader("x-requested-with");
			if ((G4Utils.isEmpty(userInfo)) && (!uri.equals("/login.ered"))
					&& (this.enabled)) {
				if (G4Utils.isEmpty(isAjax)) {
					response.getWriter().write(
							"<script type=\"text/javascript\">parent.location.href='"
									+ ctxPath
				...
				this.log.warn("警告:非法的URL请求已被成功拦截,请求已被强制重定向到了登录页面.访问来源IP锁定:"
						+ request.getRemoteAddr() + " 试图访问的URL:"
						+ request.getRequestURL().toString() + "?reqCode="
						+ request.getParameter("reqCode"));
				return;
			}
			if ((G4Utils.isNotEmpty(isAjax))
					&& (!uri.equals("/login.ered"))
					&& ((request.getParameter("loginuserid") == null) || (!request
							.getParameter("loginuserid").equals(
									userInfo.getUserid())))) {
				response.sendError(998);
				this.log.error("当前会话和登录用户会话不一致,请求被重定向到了登录页面");
				return;
			}
	...
	private void saveEvent(HttpServletRequest request, BigDecimal costTime) {
		...
		if (G4Utils.isNotEmpty(menuid)) {
			...
			String menuname = ((BaseDto) g4Dao.queryForObject(
					"Resource.queryEamenuByMenuID", menuid))
			...
			String msg = userInfo.getUsername() + "[" + userInfo.getAccount()
					+ "]调用了Action方法[" + request.getParameter("reqCode") + "]";
			dto.put("description", msg);
			this.log.info(msg + ";请求路径[" + request.getRequestURI() + "]");
		}
