...
function addTab(url,name,menuid,pathCh,icon){
  ...
  if (!n) {
     ...
     n = mainTabs.add({
       ...
       closable:true,
       layout:'fit',
       listeners: {activate: function(){Ext.getCmp('centerPanel').setTitle(pathCh)},'beforeclose': conrirmTab},
       html:'<iframe scrolling="auto" id="if'+id+'"  name="if'+id+'" frameborder="0" width="100%" height="100%" src='+url+'></iframe>'
...
//alert("aaa"+e.id);
//alert(document.getElementById("iftab_id_010403").href);
//iftab_id_010403.colseMyWindows();
//document.getElementById("iftab_id_010403").body.close;
//alert("aaab");
//wguSessionTerminate( event, gSessInfo);
}
// 解决Iframe IE加载不完全的问题
