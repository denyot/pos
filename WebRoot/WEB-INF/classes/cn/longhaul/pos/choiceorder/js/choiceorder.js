			...
			if (orderType == 1) {
				...
				if ($("#toneType").val() == '') {
					alert("请选择石料类型!");
						$("#toneType").focus();
					return;
				} else if ($("#toneColor").val() == '' && $("#toneColor").find("option").length>1) {
					alert("请选择石料颜色!");
					$("#toneColor").focus();
					return;
				} else if ($("#toneNeatness").val() == '' && $("#type").val() == "X1") {
					alert("请选择石料净度!");
					$("#toneNeatness").focus();
					return;
...
//		var goodTotalWeight = "<div id=goodTotalWeight" + rownum + ">" + Number($("#goodTotalWeight").val()) + "</div>";
		...
		// 工艺
		var technics = "<div id=technics" + rownum + " style='display:none'>" + ($("#technics").val() != "" ? ($("#technics").val()+"|-|" + $('#technics').find("option:selected").text()) : "") + "</div><div id='technicsShow"+rownum +"'>"
		...
		// + "</div>";
		"<div id=certificate" + rownum + " style='display:none'>" + ($("#certificate").selectedValuesString() != "" ? ($("#certificate").selectedValuesString() + "|-|" + $('#certificate').text()) : "") + "</div><div id='certificateShow"+rownum +"'>"
...
//		alert(totalsubtotals[1]);
		...
		if (currType == "X1" || currType == "X2" || currType == "X3" || currType == "X4")
...
//								var goldweightTo = Number($("#goldweightTo").val());
								...
								var littleTotal = parseInt(Number(number) * Number(goldWeights[0])*drjj*0.01)*100 + '-' + parseInt(Number(number) * Number(goldWeights[0])*drjj*0.01)*100;// 小计金额
								$(currentTr).find("div[id^='littleTotal']").text(littleTotal);
...
//			alert(littleTotal1);
	...
	function getTotalGold(){
		...
		for(var i = 0;i<numbers.size();i++){
			totalNumber = totalNumber + Number($(numbers[i]).text());
			totalGoldweight = totalGoldweight + Number($(goldweight[i]).text());
		}
		totalnumber.text(totalNumber);
		...
		$.ajax({
			...
			success : function(retdata){
				...
				for(var i = 0 ; i < retdata.length;i++){
					$("#matnrselect").append($("<option "+(oldMatnr == retdata[i] ? "checked" : "")+">"+ retdata[i] +"</option>"));
				}
				getmatnrbyuser();
			},
			failure : function(e){
