			...
			if (charg != "" || matnr != "") {
				$.ajax({
					...
					data : 
						...
						lessToneType+"&toneColor=" + toneColor + "&toneNeatness=" + toneNeatness,
			...
			if (orderType == 1) {
				...
				if ($("#toneType").get(0).selectedIndex == 0) {
					alert("请选择石料类型!");
						$("#toneType").focus();
					return;
				} else if ($("#toneColor").get(0).selectedIndex == 0 && $("#toneColor").find("option").length>1) {
					alert("请选择石料颜色!");
					$("#toneColor").focus();
					return;
				} else if ($("#toneNeatness").get(0).selectedIndex == 0 && $("#type").val() == "X1") {
					alert("请选择石料净度!");
					$("#toneNeatness").focus();
					return;
			...
			if (charg != "" || matnr != "") {
				$.ajax({
					...
					data : 
						...
						$.trim(lessToneType)+"&toneColor=" + toneColor + "&toneNeatness=" + toneNeatness,
//					"&toneType=" + $("#toneType").val() + "&gemweight=" + $("#gemweight").val() + "&toneColor=" + $("#toneColor").val() + "&toneNeatness=" + $("#toneNeatness").val() + "&technics="
//							+ $("#technics").val() + "&extwg=" + $("#extwg").val() + "&matkl=" + $("#matkl").val()+"&matnr=" + matnr+"&charg=" + charg,
					success : function(data) {
						var oldRetailPrice = Number($("#oldRetailPrice").val());
	...
	function getTotal(){
		...
		for(var i = 0;i<numbers.size();i++){
			...
			littleTotal0 = littleTotal0 + Number(littleTotals[0])*count;
			littleTotal1 = littleTotal1 + Number(littleTotals[1])*count;
