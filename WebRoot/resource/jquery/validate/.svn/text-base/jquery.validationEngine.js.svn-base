	...
	var methods = {
		...
		_onFieldEvent : function(event) {
			...
			window.setTimeout(function() {
				/*
				备注：原语句是methods._validateField(field, options);
				由于要满足在客户blur时不提示验证信息但要将原有的验证信息隐藏掉，所以传一个false过去，告诉blur只隐藏验证信息
				*/
				methods._validateField(field, options, null, false);
		...
		_onSubmitEvent : function() {
			...
			var r = methods._validateFields(form, false);
		 ...
		 */
		_validateField : function(field, options, skipAjaxValidation, showTip) {
			if (!field.attr("id"))
				$
			...
			for ( var i = 0; i < rules.length; i++) {
	          ...
	          if (errorMsg !== undefined) {
				...
				case "groupRequired":
					...
					var firstOfGroup = field.closest("form").find(classGroup)
							.eq(0);
				...
				case "dateRange":
					...
					var firstOfGroup = field.closest("form").find(classGroup)
							.eq(0);
					var secondOfGroup = field.closest("form").find(classGroup)
							.eq(1);
					/*
					if (firstOfGroup[0] != field[0]) {
			...
			//            }
			/*
				备注：原语句是if (options.isError)
				由于要满足在客户blur时不提示验证信息但要将原有的验证信息隐藏掉，所以如果showTip无值时提示验证信息，有值时隐藏验证信息
			*/
			if (options.isError && showTip == null) {
		...
		_required : function(field, rules, i, options) {
			...
			case "select-multiple":
				// added by paul@kinetek.net for select boxes, Thank you
				if (!field.find("option:selected").val().trim())
					return options.allrules[rules[i]].alertText;
				break;
			}
		},
		...
		_equals : function(field, rules, i, options) {
            ...
            if(field.attr("type") == "password") {
                var fieldValueForEq = field.val();
            } else {
                var fieldValueForEq = $.trim(field.val());
            }
            var oldFildForEq = $("#" + equalsField);
            if(oldFildForEq.attr("type") == "password") {
                var oldFildValueForEq = oldFildForEq.val();
            } else {
                var oldFildValueForEq = $.trim(oldFildForEq.val());
            }
            
            if (fieldValueForEq != oldFildValueForEq)
		...
		_ajax : function(field, rules, i, options) {
			...
			if (extraDataDynamic) {
				...
				for ( var i = 0; i < domIds.length; i++) {
					var id = domIds[i];
					if ($(id).length) {
						var inputValue = field.closest("form").find(id).val().trim();
