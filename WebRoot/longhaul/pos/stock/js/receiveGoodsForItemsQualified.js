...
Ext.onReady(function() {
	...
	var store = new Ext.data.Store({
		...
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getReceiveStoreHeadForQualified'
		}),
		// 数据读取器
	...
	function getCheckboxValues() {
		...
		for ( var i = 0; i < rows.length; i++) {
			...
			itemData += "'manageresult':'" + rows[i].get("manageresult") + "',";
			if (i == rows.length - 1) {
				itemData += "}";
			} else {
				itemData += "},";
		...
		// var meterialdocument =
		// Ext.getCmp('meterialdocument').getValue();
		...
		Ext.Msg.confirm("提示", "确定提交吗？", function(btn, text) {
			if (btn == "yes") {
			      ...
			      myMask.show();
				Ext.Ajax.request({
					url : 'stockSystem.ered?reqCode=submitItemsManageResult',
					method : 'post',
					params : {
						itemData : itemData,
						id : myid,
					...
					success : function(data) {
						...
						if (retData.success != null) {
							Ext.Msg.alert("提示成功", retData.success, function() {
								thirdWindow.hide();
								queryCatalogItem();
							});
							...
							Ext.Msg.alert("提示失败", "出现未知错误，请联系管理员！", function() {
							});
						}
	...
	function submitHeadReason() {
		...
		for ( var i = 0; i < rows.length; i++) {
			...
			var lgort = rows[i].get("lgort");
			var goodtype = rows[i].get("goodtype");
			// alert(headmanageresult);
			if (manageresult != null && manageresult != '') {
				chargitem = chargitem + "\"" + count + "\":";
				chargitem = chargitem + "{\"id\":\"" + rows[i].get("id") + "\",";
				// chargitem = chargitem + "\"needvalified\":\"" + 6 +
				// "\",";

				chargitem = chargitem + "\"charg\":\"" + rows[i].get("charg") + "\",";

				chargitem = chargitem + "\"goodscount\":\"" + rows[i].get("menge") + "\",";

				chargitem = chargitem + "\"matnr\":\"" + rows[i].get("matnr") + "\",";

				chargitem = chargitem + "\"lgort\":\"" + lgort + "\",";

				chargitem = chargitem + "\"goodtype\":\"" + goodtype + "\",";

				chargitem = chargitem + "\"manageresult\":\"" + manageresult + "\"}";
				chargitem = chargitem + ",";
				count++;
			}
	...
	function getTotal() {
		...
		for ( var i = 0; i < rows1.length; i++) {
			count = count + Number(rows1[i].get("chargcount"));
			total = total + Number(rows1[i].get("kbetr") * rows1[i].get("chargcount"));
			chargwidth = chargwidth + Number(rows1[i].get("hpzl") * rows1[i].get("chargcount"));
		}
