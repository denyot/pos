...
Ext.onReady(function() {
	Ext.data.Connection.prototype.timeout='30000000';
...
//	});
	...
	var store = new Ext.data.Store({
		// 获取数据的方式
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getStockList'
		}),
		// 数据读取器
		reader : new Ext.data.JsonReader({
	...
	store.on('beforeload', function() {
		this.baseParams = {
			...
			lgort : lgort_combo.getValue(),
		 	status : posttype_combo.getValue()
	...
	var pagesize_combo = new Ext.form.ComboBox({
		...
		store : new Ext.data.ArrayStore({
			fields : [ 'value', 'text' ],
			data : [ [ 10, '10条/页' ], [ 20, '20条/页' ], [ 50, '50条/页' ], [ 100, '100条/页' ], [ 250, '250条/页' ], [ 500, '500条/页' ], [ 999999999, '更多' ]  ]
		}),
		valueField : 'value',
		displayField : 'text',
		value : '500',
	...
	var kondm_combo = new Ext.ux.form.LovCombo({
		name : 'kondm',
		triggerAction : 'all',
		mode : 'local',
		fieldLabel : '品类',
	...
	var tbar = new Ext.Toolbar({
		items : [ {
			...
			width : 60,
			id : "matnr"
		}, '-', lgort_combo, goodtype_combo, '-', posttype_combo, {
			text : '查询',
			iconCls : 'page_refreshIcon',
			handler : function() {
				queryCatalogItem();
...
//				var kondm = kondm_combo.getValue();
...
//				var lgort = lgort_combo.getValue();
				...
				var matnr = Ext.getCmp("matnr").getValue();
				var lgort = lgort_combo.getValue();
			 	var status = posttype_combo.getValue()
				
				
				var url = 'stockSystem.ered?reqCode=exportExcel&ifEnough=' + ifEnough + "&charg=" + charg + "&kondm=" + kondm + "&matnr=" + matnr + "&lgort=" + lgort+ "&status=" + status
				exportExcel(url);
			}
		}, {
			text : '更新价格状态',
	...
	grid.on("cellclick", function(pGrid, rowIndex, columnIndex, e) {
		...
		if (fieldName == 'showpic' && columnIndex == 20) {
	...
	var picpanel = new Ext.Panel({
		layout : 'fit',
		html : "<img id='goodimage' src='./images/sample1.gif'" + "onError=\"this.src='./images/sample1.gif'\"/>"
	});
	...
	var picWindow = new Ext.Window({
		title : '<span class="commoncss">产品图片</span>', // 窗口标题
		// layout : 'fit', // 设置窗口布局模式
		 width : 600, // 窗口宽度
		iconCls : 'window_caise_listIcon', // 按钮图标
		 height : 400, // 窗口高度
			...
			if (e == 'yes') {
				...
				for ( var i = 0; i < rows.length; i++) {
					if (rows[i].get("kbstat") == '冻结') {
						...
						info = info + "\"kbetr\":\"" + rows[i].get("kbetr") + "\",";
