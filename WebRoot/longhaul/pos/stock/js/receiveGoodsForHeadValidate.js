...
Ext.onReady(function() {
	...
	// sm,
	{
				...
				header : '收货门店', // 列标题
				dataIndex : 'werksStr', // 数据索引:和Store模型对应
				sortable : true,// 是否可排序
				width : 140
			}, {
				header : '收货日期', // 列标题
				dataIndex : 'recievedate', // 数据索引:和Store模型对应
				sortable : true,// 是否可排序
				width : 80
			}, {
				header : '物料编码',
	...
	var store = new Ext.data.Store({
		...
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getReceiveStoreHeadForHeadValidate'
		}),
		// 数据读取器
	...
	var tbar = new Ext.Toolbar({
		items : [ {
			...
			handler : function() {
				exportExcel('stockSystem.ered?reqCode=exportReceiveStoreHeadForHeadValidateExcel');
			}
		} ]
	});
	...
	function submitHeadManage() {
		...
		var count = 0;
		for ( var i = 0; i < rows.length; i++) {
			var headmanage = rows[i].get("headmanage");
			// alert(headmanage);
			if (headmanage != null && headmanage != '') {
				...
				chargitem = chargitem + "\"headmanage\":\"" + headmanage
						+ "\"}";
				chargitem = ((i == (rows.length - 1)) ? chargitem
						: (chargitem + ","));
				count++;
			}
		}
		...
		Ext.Msg.confirm("提示", "确定提交吗？", function(btn, text) {
					if (btn == "yes") {
					      ...
					      myMask.show();
						Ext.Ajax.request({
									url : 'stockSystem.ered?reqCode=submitHeadManage',
									method : 'post',
									params : {
										chargitem : chargitem
									},timeout : 12000000,
