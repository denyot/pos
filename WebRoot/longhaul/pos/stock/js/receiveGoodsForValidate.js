...
Ext.onReady(function() {
	...
	var store = new Ext.data.Store({
				...
				proxy : new Ext.data.HttpProxy({
							url : 'stockSystem.ered?reqCode=getReceiveStoreHeadForValidate'
						}),
				// 数据读取器
...
//						},
						'-', {
							...
							handler : function() {
								exportExcel('stockSystem.ered?reqCode=exportReceiveStoreHeadForValidateExcel');
							}
						}]
			});
	...
	function queryCatalogItem() {
		store.load({
					params : {
						start : 0,
		                limit : bbar.pageSize,
		               	recieveDateFrom : Ext.getCmp("recieveDateFrom").getValue(),
						recieveDateTo : Ext.getCmp("recieveDateTo").getValue()
					},
					callback : function(records, options, success) {
						getTotal();
						if (success == true) {
...
//		});
	...
	function getCheckboxValues() {
		...
		if(itemData == ''){
			Ext.Msg.alert("提示","您没有输入任何处理方案,请输入后再提交!",function(e){
			});
			return;
		}
		itemData = "{"+itemData+"}";
					...
					if (btn == "yes") {
						Ext.Ajax.request({
									...
									success : function(data) {
										Ext.Msg.alert("提示", data.responseText,function(e){
											thirdWindow.hide();
											queryCatalogItem();
										});
									}
	...
	function submitHeadManage(){
		...
		for (var i = 0; i < rows.length; i++) {
			...
			if(manage != null && manage != '') {
				...
				//chargitem = chargitem + "\"needvalified\":\"" + 4 + "\",";
				manage.replace("\n","<br/>");
				//alert(manage);
				chargitem = chargitem + "\"manage\":\"" + manage + "\"}";
				chargitem = chargitem + ",";
	...
	function getTotal() {
		...
		for ( var i = 0; i < rows1.length; i++) {
			...
			total = total + Number(rows1[i].get("kbetr"))* Number(rows1[i].get("menge"));
	...
	function getCount() {
		...
		var totalweight = 0;
		for ( var i = 0; i < rows1.length; i++) {
			count = count + parseInt(rows1[i].get("count")+'');
