...
Ext.onReady(function() {
	...
	werksInfo.load();
	var areaInfo = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getAreaInfo'
		}),
		reader : new Ext.data.JsonReader({}, [ {
			name : 'vkbur'
		}, {
	...
	var cm = new Ext.grid.ColumnModel([ rownum, {
		...
		header : '出库日期', // 列标题
		dataIndex : 'datum', // 数据索引:和Store模型对应
		sortable : true,// 是否可排序
		width : 80
	}, {
		header : '收货日期', // 列标题
	...
	var store = new Ext.data.Store({
		// 获取数据的方式
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getReceiveStoreHead'
		}),
		// 数据读取器
		reader : new Ext.data.JsonReader({
	...
	store.on('beforeload', function() {
		this.baseParams = {
			...
			toWerks : werks_combo.getValue(),
			area : area_combo.getValue(),
	...
	var werks_combo = new Ext.form.ComboBox({
		id : 'werkinfo',
		triggerAction : 'all',
		mode : 'local',
		store : werksInfo,
	...
	var area_combo = new Ext.form.ComboBox({
		id : 'areainfo',
		triggerAction : 'all',
		mode : 'local',
		store : areaInfo,
	...
	var tbar = new Ext.Toolbar({
		items : [ {
			...
			format : 'Y-m-d',
			id : "totime"
		}, '-', goodtype_combo, '-', posttype_combo, '-', werks_combo, '-',
				area_combo, '-', {
					text : '查询',
					iconCls : 'page_refreshIcon',
					handler : function() {
