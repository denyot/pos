...
Ext.onReady(function() {
	...
	var diffInfo = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getTakDiffType'
		}),
		reader : new Ext.data.JsonReader({}, [ {
			name : 'difftype'
		} ])
...
//	}, {
//		header : '石料净度',
//		dataIndex : 'toneNeatNessStr',
//		sortable : true,
//		width : 120
//	}, {
//		header : '石料颜色',
//		dataIndex : 'toneColorStr',
//		sortable : true,
//		width : 120
//	}, {
//		header : '货品证书',
...
//		width : 50
	...
	var storeWaitToTakingInfo = new Ext.data.Store({
		// 获取数据的方式
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getStockTakingInfo'
		}),
		// 数据读取器
		reader : new Ext.data.JsonReader({
			id : 'callTime'
...
//		width : 140
	...
	var chargLgortInfo = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getChargLgortInfo'
		}),
		reader : new Ext.data.JsonReader({}, [ {
			name : 'werks'
		}, {
	...
	// chargLgortInfo.load(); // 如果mode : 'local',时候才需要手动load();
	
	var charglgort_combo = new Ext.form.ComboBox({
		triggerAction : 'all',
		store : chargLgortInfo,
		emptyText : '库位',
		fieldLabel : "盘点库位",
		displayField : 'lgobe',
		valueField : 'lgort',
	...
	var stockTakingIdInfo = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getStockTakingId'
		}),
		reader : new Ext.data.JsonReader({}, [ {
			name : 'id'
		}, {
	...
	var tbar = new Ext.Toolbar({
		...
		'-', {
			...
			handler : function() {
				if (storeWaitToTakingInfo.data.items.length != 0) {
			...
			handler : function() {
				storeTakedInfo.load();
				if (storeTakedInfo.data.items.length != 0) {
					takedWindow.show();
					getCount1();
				}
			}
	...
	grid.on("cellclick", function(pGrid, rowIndex, columnIndex, e) {
		...
		if (fieldName == 'submititem' && columnIndex == 2) {
			var remark = record.get('remark');
			remark = remark == null ? '' : remark;
			var charg = record.get('charg');
			var matnr = record.get('matnr');
			var lgort = record.get('lgort') == null ? currLgort : record.get('lgort');
			var difftype = record.get('difftype');
			difftype = difftype == null ? '' : difftype;
			...
			for ( var i = 0; i < rows.length; i++) {
				var rowscharg = rows[i].get("charg");
				rowscharg = rowscharg == null ? '' : rowscharg;
				if (rowscharg == charg) {
			...
			for ( var i = 0; i < rows2.length; i++) {
				var rowscharg = rows2[i].get("charg");
				rowscharg = rowscharg == null ? '' : rowscharg;
				if (rowscharg == charg) {
					ischecked = true;
					checkedIndex = i;
			...
			// if (rowscharg == charg && rowsmatnr == matnr && rowslgort ==
			// lgort) {
			// Ext.Ajax.request({
			// url :
			// 'stockSystem.ered?reqCode=submitStockTakingInfo&option=user&postType=1',
			// method : 'post',
			// params : {
			// id : currentStockTakingId,
			// charg : charg,
			...
			Ext.Ajax.request({
				url : 'stockSystem.ered?reqCode=submitStockTakingInfo&option=user&postType=1&random=' + Math.random(),
				method : 'post',
				params : {
					id : currentStockTakingId,
					charg : charg,
	...
	var lgortInfoForm = new Ext.form.FormPanel({
		id : 'lgortInfoForm',
		name : 'lgortInfoForm',
		labelWidth : 110, // 标签宽度
		frame : true, // 是否渲染表单面板背景色
		...
		items : [ {
			...
			items : [ {
				...
				items : [ {
					...
					anchor : '90%',
					handler : function(){
						var lgort = charglgort_combo.getValue();
						if(lgort == ''){
							Ext.Msg.alert("提示","请选择库位！",function(e){
							});
						}else{
							stockWindow.hide();
							currLgort = lgort;
	...
	var orderStockTakingWindow = new Ext.Window({
		title : '<span class="commoncss">生成盘点</span>', // 窗口标题
		// layout : 'fit', // 设置窗口布局模式
		width : 600, // 窗口宽度
		iconCls : 'window_caise_listIcon', // 按钮图标
		// height : 400, // 窗口高度
	...
	var waittakingWindow = new Ext.Window({
		title : '<span class="commoncss">待盘清单</span>', // 窗口标题
		// layout : 'fit', // 设置窗口布局模式
		width : 900, // 窗口宽度
		iconCls : 'window_caise_listIcon', // 按钮图标
		height : 400, // 窗口高度
	...
	var takedWindow = new Ext.Window({
		title : '<span class="commoncss">已盘清单</span>', // 窗口标题
		// layout : 'fit', // 设置窗口布局模式
		width : 900, // 窗口宽度
		iconCls : 'window_caise_listIcon', // 按钮图标
		height : 400, // 窗口高度
	...
	var stockWindow = new Ext.Window({
		title : '<span class="commoncss">请选择批次库位</span>', // 窗口标题
		// layout : 'fit', // 设置窗口布局模式
		width : 600, // 窗口宽度
		iconCls : 'window_caise_listIcon', // 按钮图标
		height : 100, // 窗口高度
	...
	function getRecord() {
		...
		if (myrows.length != 0) {
			...
			difftype = difftype == null ? '' : difftype;
			var charg = record.get('charg');
			var charg_o = record.get('charg_o');
			//alert(charg_o);
			...
			var iswaitingchecking = false;// 是否在未盘之中
			var waitingcheckingIndex = 0;
			ischecked = false;// 是否在已盘之中
			var checkedIndex = 0;
			var rows = storeWaitToTakingInfo.data.items;

			for ( var i = 0; i < rows.length; i++) {
				var rowscharg = rows[i].get("charg");
				var rowscharg_o = rows[i].get("charg_o");
				rowscharg = rowscharg == null ? '' : rowscharg;
	            if( rowscharg_o == charg){
	                charg = rowscharg;
	            }
				if (rowscharg == charg ) {
					iswaitingchecking = true;
			...
			for ( var i = 0; i < rows2.length; i++) {
				var rowscharg = rows2[i].get("charg");
				var rowscharg_o = rows2[i].get("charg_o");
				rowscharg = rowscharg == null ? '' : rowscharg;
					...
					if (retData.success != null) {
						...
						if (iswaitingchecking) {
							...
							for ( var i = 0; i < rows2.length; i++) {
								var rowscharg = rows2[i].get("charg");
								rowscharg = rowscharg == null ? '' : rowscharg;
								if (rowscharg == record2.get('charg')) {
									ischecked = true;
									isNeedCheck = true;
									checkedIndex = i;
									break;
						...
						var charg = Ext.getCmp("charg").getValue();
						var matnr = Ext.getCmp("charg").getValue();
						if (charg == '' && matnr == '') {
							...
							if (isNeedCheck) {
								Ext.Msg.alert("提示", "该批次库存数为0，请选择该批次所在库位信息！", function(e) {
									chargLgortInfo.load({
										params:{
											charg : charg,
											id : currentStockTakingId
										...
										callback : function(records, options, success) {
											if (success == true) {
												getTotal();
												if(chargLgortInfo.data.items.length <= 1){
													currLgort = chargLgortInfo.data.items[0].get("lgort");
												}else
													stockWindow.show();
											} else
												Ext.Msg.alert('提示', '出现错误');
							...
							if (ischecked) {
								Ext.Msg.alert("提示", "该批次已经盘点，请注意！", function(e) {
									return false;
								});
							}
							...
							if (!checkRepeat('charg', charg)) {
								...
								Ext.Ajax.request({
									url : 'stockSystem.ered?reqCode=getchargforstocktaking&option=user&postType=1&random=' + Math.random(),
									method : 'post',
									params : {
										id : currentStockTakingId,
										charg : charg,
			...
			if (charg == '' && matnr == '') {
				...
				if (isNeedCheck) {
					Ext.Msg.alert("提示", "该批次库存数为0，请注意！", function(e) {
						chargLgortInfo.load({
							params:{
								charg : charg,
								id : currentStockTakingId
	...
	function checkRepeat(recordName, recordValue) {
		...
		return true;
		var myitems = storeTakedInfo.data.items;
		if (myitems.length == 0)
			return true;
	...
	function submitStockTakingTime() {
		...
		Ext.Ajax.request({
			url : 'stockSystem.ered?reqCode=submitStockTakingTime&random=' + Math.random(),
			params : {
				date : date,
				fdate : Ext.getCmp('stockDate').getValue()
			},
			timeout : 12000000,
			type : 'post',
			dataType : 'json',
	...
	function getTotal() {
		...
		var count1 = storeWaitToTakingInfo.data.items.length;
		var count2 = storeTakedInfo.data.items.length;
		var count = count1 + count2;
		Ext.getCmp("count").setText(parseInt(count));
	}

	function getCount() {
		var rows1 = gridnotaking.store.data.items;
		var count = 0;
		var total = 0;
		for ( var i = 0; i < rows1.length; i++) {
			var num = Number(rows1[i].get("labst"));
			var meins = rows1[i].get("meins");
			count += 1;	
	...
	function getCount1() {
		var rows1 = gridtaked.store.data.items;
		var count = 0;
		var total = 0;
		for ( var i = 0; i < rows1.length; i++) {
			var num = Number(rows1[i].get("labst"));
		...
		Ext.getCmp("kbetr1").setText(total);
		Ext.getCmp("menge1").setText(parseInt(count));
	}
	...
	function submitStockTaking() {
		...
		Ext.Ajax.request({
			url : 'stockSystem.ered?reqCode=submitStockTakedInfo&option=user&postType=1&random=' + Math.random(),
			params : {
				id : currentStockTakingId
			},timeout : 12000000,
			success : function(data) {
				...
				if (retData.success != null) {
					Ext.Msg.alert("提示成功", retData.success, function(e) {
						...
						stockTakingIdInfo.load();
						stockTakingIdInfo_combo.setValue('');
						Ext.getCmp("noStockTakingCount").setText("未盘数量：");
						Ext.getCmp("StockTakedCount").setText("已盘数量：");
					});
				} else if (retData.error != null) {
	...
	function delStockTaking() {
		...
		Ext.Msg.confirm("提示", "确定删除吗？", function(e) {
			if (e == 'yes') {
				...
				Ext.Ajax.request({
					url : 'stockSystem.ered?reqCode=delStockTakingInfo&option=user&postType=1&random=' + Math.random(),
					params : {
						id : myid
					},timeout : 12000000,
					success : function(data) {
