...
//							name : 'total'
	...
	var cm2 = new Ext.grid.ColumnModel([
			...
			var sealmethodStore = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : 'reportSystem.ered?reqCode=getSealMethod'
						}),
				reader : new Ext.data.JsonReader({}, [{
									name : 'zxsfs'
								}, {
	...
	// 收货清单
	var cmedit = new Ext.grid.ColumnModel([
...
//						}]
	...
	var tbar = new Ext.Toolbar({
				items : [{
							...
							tooltip : '导出Excel',
							handler : function() {
								exportExcel('reportSystem.ered?reqCode=outStockHeaderExportExcel&type=db&status=0&outtimeb='+
										Ext.getCmp('outtimeb').formatDate(Ext.getCmp('outtimeb').getValue())+'&outtimee='+
										Ext.getCmp('outtimee').formatDate(Ext.getCmp('outtimee').getValue())+'&outwerks='+
										Ext.getCmp('outwerks').getValue()+'&inwerks='+Ext.getCmp('inwerks').getValue()+'&start=0&limit=1000');
							}
						} ]
			});
	...
	grid.on("cellclick", function(pGrid, rowIndex, columnIndex, e) {
				...
				if (fieldName == 'showdetail' && columnIndex == 1) {
					...
					var outid = record.get('outid');
					var status = Number(record.get('status'));
					...
					if (status <= 3 && status != 2) {
						...
						Ext.Msg.alert("提示", "货品门店已经发货，不能修改！");
					}
				}
	...
	function cancleStock() {
		...
		for (var i = 0; i < rows.length; i++) {
			...
			updateItem += "'bqj':'"
					+ ((rows[i].get("bqj") != null && rows[i].get("bqj") != '')
							? rows[i].get("bqj")
							: '0') + "',";
			updateItem += "'outwerkssealmethod':'"
					+ (rows[i].get("sealmethod") == null ? '' : rows[i]
							.get("sealmethod")) + "',";
			updateItem += "'inwerkssealmethod':'"
					+ ((rows[i].get("inwerkssealmethod") == null || rows[i]
							.get("inwerkssealmethod") == '') ? sealMethod : rows[i]
							.get("inwerkssealmethod")) + "',";
			updateItem += "'ifneedprintLabel':'"
					+ ((rows[i].get("ifneedprintlabel") == null || rows[i]
							.get("ifneedprintlabel") == '')
							? ifneedPrintLabel
							: rows[i].get("ifneedprintlabel")) + "',";
			updateItem += "'ifneedchangPrice':'"
					+ ((rows[i].get("ifneedchangprice") == null || rows[i]
							.get("ifneedchangprice") == '')
							? ifneedUpdatePrice
							: rows[i].get("ifneedchangprice")) + "',";
		...
		Ext.Msg.confirm("提示", "确定提交吗？", function(btn, text) {
					if (btn == "yes") {
						...
						myMask.show();
						Ext.Ajax.request({
									url : 'reportSystem.ered?reqCode=submitValidate',
									method : 'post',
									params : {
										outid : currentId,
										rejreason : rejreason,
