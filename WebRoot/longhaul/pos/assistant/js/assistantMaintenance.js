		...
		.onReady(function() {
			...
			var store = new Ext.data.Store({
				...
				proxy : new Ext.data.HttpProxy({
					url : 'assistantSystem.ered?reqCode=getAssistansInfo'
				}),
				// 数据读取器
			...
			function queryBalanceInfo() {
				...
				store.load({
					params : {
						...
						statuscombo : status_combo.getValue()
					},
			...
			var secondWindow = new Ext.Window(
					{
						...
						buttons : [
								{ // 窗口底部按钮配置
									...
									handler : function() { // 按钮响应函数
										if(Ext.getCmp('status1').getValue()== ''){
											Ext.Msg.alert('提示','请选择状态！');
											return false;
										}
			...
			function setSecondFormText() {
				...
				Ext.getCmp("rzsj").setValue(clickone.get("rzsj"));
			}
