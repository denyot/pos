...
Ext.onReady(function() {
	...
	var store_yy = new Ext.data.Store({   
        proxy: new Ext.data.HttpProxy({ url: "orderSystem.ered?reqCode=getOrderReason&postType=1" }) ,
...
//		frame : true,
	...
	function getReceiveStoreDetail(){
		Ext.Ajax.request({
					url    : "../stock/stockSystem.ered?reqCode=getStockList",
					method : 'post', 
					async  : true, 	
					params : {charg : Ext.getCmp("searchCharg").getValue(),ifEnough:'1',start:0,limit:1},
					success : function(respose) {
						var roots = Ext.decode(respose.responseText);
						var charginfo = roots.ROOT[0];
						if(!validateCharg(charginfo)){
							//chargDetailForm.getForm().reset();//清空
							return false;
						}
						...
						var labst = Ext.getCmp("detaillabst");
						var hpzl = Ext.getCmp("detailhpzl");
						//控制输入
						if(charginfo.meins=="PC"){
							hpzl.disable();
							labst.enable();
						}else{
							labst.disable();
						...
						Ext.getCmp("detailCharg").setValue(charginfo.charg);
						labst.setValue(charginfo.labst);
						hpzl.setValue(charginfo.hpzl);
						Ext.getCmp("detailMlabst").setValue(charginfo.labst);//max
						Ext.getCmp("detailMhpzl").setValue(charginfo.hpzl);//max
						Ext.getCmp("detailmatnr").setValue(charginfo.matnr);
						Ext.getCmp("detailmaktx").setValue(charginfo.ztxt1);
						Ext.getCmp("detailmeins").setValue(charginfo.meins);
						Ext.getCmp("detaillgort").setValue(charginfo.lgort);
					    Ext.getCmp("detailKbetrb").setValue(charginfo.kbetr);//标签价
					    Ext.getCmp("detailTs").setValue("zzzk3");
					    Ext.getCmp("detailKl").setValue("01");
					    Ext.getCmp("detailpicurl").setValue(charginfo.zmatnrt);
					    
						 //黄金获取工费
						 if(charginfo.meins=="G"&&!charginfo.kbetr){
						 	//工费
						 	setGongFei(charginfo);
						 	//金价
						 	setMeiZttj(charginfo);
						 }else{
						 	//设置常规折扣
						     getzzk1(charginfo);
						 }
						 setPicUrl(charginfo);//设置图片路径
						//设置实收
						setKbetr();
	...
	function validateCharg(charginfo){
		if(!charginfo){
			...
			return false;
		}else if (charginfo.kbstat != "正常"){
			Ext.Msg.alert("提示","价格被冻结！可明日录单。");
			return false;
