				...
				"longhaul/pos/order/orderSystem.ered?reqCode=orderSystem&postType=1&random="
					...
					$.each(data, function(key, val) {
						...
						row = row + "<td><a href='" + _herf
								+ "' target='_blank'><div id=sapsalesorderid"
								+ key + ">" + sapsalesorderid
								+ "</div></a></td>";
						row = row + "<td><div id='ordertype" + key
								+ "' style='display : none;'>" + val.ordertype
								+ "</div>" + val.ordertypetext + "</td>";
						row = row + "<td>" + val.saledate.substring(0, 10)
								+ "</td>";
						...
						// deliveryordernumbert = val.deliveryordernumber ==
						// null
						// ? ""
						...
						row = row
								+ "<td style='display:none'><div id=operatedatetime"
								+ key + " > " + val.operatedatetime
								+ " </div></td>";
						row = row + "</tr>"
					...
					row = row + "<td>" + "总金额：" + "</td>";
					row = row + "<td align='right'>" + totalMoney.toFixed(2)
							+ "</td>";
					row = row + "<td>" + "</td>";
					row = row + "</tr>"
	...
	function getKondm() {
		$.getJSON(
				"longhaul/pos/stock/stockSystem.ered?reqCode=getKondmInfo&postType=1&random="
					...
					$.each(data, function(key, val) {
								var option = $("<option value='" + val.kondm
										+ "'>" + val.vtext + "</option>");
								$("#kondm").append(option);
							});
				});
				...
				"longhaul/pos/order/orderSystem.ered?reqCode=getOrderDetail&postType=1&random="
					...
					$.each(data, function(key, val) {
						...
						row = row
								...
								+ key + "'/></td>";
						row = row + "<td style='display:none;'><a href='"
								+ _herf
								+ "' target='_blank'><div id=salesorderid"
								+ key + "> " + val.salesorderid
								+ "</div></a></td>";
						...
						totalmoney = val.totalmoney;
						totalmoney = Number(totalmoney).toFixed(2);
						row = row + "<td align='right'>" + val.totalamount
								+ "</td>";
						row = row + "<td align='right'>" + val.tagprice
								+ "</td>";
						row = row + "<td align='right'>" + val.goldprice
								+ "</td>";
						row = row + "<td align='right'>" + val.batchnumber
								+ "</td>";
						...
						row = row
								...
								+ "<img src='sappic/"
								+ val.zmatnrt
								+ "' onerror='this.src=\"longhaul\/pos\/order\/images\/zjzb.gif\"' width='40px' height='40px'/></td>";
						row = row + "<td align='right'>" + val.salesquantity
								+ "</td>";

						row = row
								+ "<td align='right'>"
								+ (Number(val.tagprice) == 0
										|| Number(val.totalamount) == 0
										? ""
										: (Number(val.totalamount)
												/ val.tagprice * 100)
												.toFixed(0)) + "</td>";
						// row = row + "<td align='right'>" + val.discount1 +
						// "</td>";
						// row = row + "<td align='right'>" + val.discount2 +
						// "</td>";
						// row = row + "<td align='right'>" + val.discount3 +
						// "</td>";
						// row = row + "<td align='right'>" + val.discount4 +
						// "</td>";
						// row = row + "<td align='right'>" + val.discount5 +
						// "</td>";
						row = row + "<td align='right'>"
								+ (val.zjlbm != null ? val.zjlbm : '')
								+ "</td>";
						row = row + "<td align='right'>" + val.goldweight
								+ "</td>";
						row = row + "<td align='right'>"
								+ val.goodsprocessingfee + "</td>";
						row = row + "<td align='right'>" + val.currentintegral
								+ "</td>";
						row = row + "<td align='right'>"
								+ (val.zslbm != null ? val.zslbm : '')
								+ "</td>";
						row = row + "<td align='right'>"
								+ (val.zzlnn != null ? val.zzlnn : '')
								+ "</td>";
						row = row + "<td align='right'>"
								+ (val.labor != null ? val.labor : '')
								+ "</td>";
						row = row + "<td align='right'>"
								+ (val.zslys != null ? val.zslys : '')
								+ "</td>";
						row = row + "<td>" + val.salesclerk + "</td>";
						// deliveryordernumbert = val.deliveryordernumber ==
						// null
						...
						row = row
								...
								+ " </div></td>";
						row = row + "</tr>"
						$(row).insertAfter($("#orderlistdetailtable tr:eq("
								+ key + ")"));
						$("#searchdetail").val("明细查询");
						$("#searchdetail").attr("disabled", false);
					...
					row = row + "<td>" + "</td>";
					row = row + "<td>" + "</td>";
					row = row + "<td colspan=2 align='right'>" + "总数量："
							+ "</td>";
					row = row + "<td>" + totalCount.toFixed(0) + "</td>";
					row = row + "<td>" + "</td>";
					row = row + "<td>" + "</td>";
		...
		$
				.getJSON(
						"longhaul/pos/order/orderSystem.ered?reqCode=getOrderTypeList&postType=1&werks="
								+ WERKS + "", function(data) {
							var auart = "";
//							$("#ordertype").append($("<option value='' selected>所有订单类型</option>"));
							$.each(data, function(key, val) {
								if(val.pzdm != 'ZYS5' && val.pzdm != 'ZYS6'){
									$("#ordertype")
									.append($("<option value="
											+ val.pzdm + ">"
											+ val.pzmc + "("
...
//		if ($("#ordertype").val() == 'ZYS4' || $("#ordertype").val() == 'ZJM4' || $("#ordertype").val() == 'ZYS6' || $("#ordertype").val() == 'ZJM6') {
//			$("#del").show();
//		} else {
//			$("#del").hide();
//		}
	...
	$("#update").click(function(event) {
		...
		if ($("input[name='useroption']:checked").size() > 1) {
			...
			if (_orderflag == "NO" || _orderflag == "UO") {
				window
						.open(basepath
								+ "/longhaul/pos/order/orderSystem.ered?reqCode=orderSystemStart&postType=1&salesorderid="
								+ _salesorderid + "&WERKS=" + WERKS
								+ "&userid=" + opterator + "&ordertype="
								+ _orderType + "&password=" + password
								+ "&opmode=EDIT");
			} else {
				jAlert('订单已经上传，不允许修改!', '提示', function(r) {
						});
				return;
			}
		}
	...
	$("#del").click(function(event) {
		...
		$("input[name='useroption']:checked").each(function() {
			...
			_ordertype = $.trim($("#ordertype" + usedkey).text());
			_deliveryordernumber = $.trim($("#deliveryordernumber" + usedkey)
					.text());
			_orderflag = $("#orderflag" + usedkey).text();
			if (1==1){//_orderflag == "NO" || (_ordertype == 'ZYS4' || _ordertype == 'ZJM4' || _ordertype == 'ZYS6' || _ordertype == 'ZJM6')) {
		...
		$
				.post(
						"longhaul/pos/order/orderSystem.ered?reqCode=getOrderPageCount&postType=1&random="
							...
							$('#pageOpr').die().unbind().clearQueue()
									.jqPagination({
										// link_string : '/?page={page_number}',
										// current_page: 1, //设置当前页 默认为1
										max_page : parseInt(data), // 设置最大页
																	// 默认为1
										page_string : '当前第{current_page}页,共{max_page}页',
										paged : function(page) {
											getorderbypage(page - 1,
													$("#pageSize").val());
										}
									});
						});
		...
		$
				.post(
						"longhaul/pos/order/orderSystem.ered?reqCode=getOrderDetailForPageCount&postType=1&random="
							...
							$('#pageOprDetail').die().unbind().clearQueue()
									.jqPagination({
										// link_string : '/?page={page_number}',
										// current_page: 1, //设置当前页 默认为1
										max_page : parseInt(data), // 设置最大页
	...
	function exportOrder() {
		...
		//alert(_ordertype);
		_ordertype = _ordertype == null ? "" : _ordertype;
		_ordertype = _ordertype =="所有订单类型" ? "" : _ordertype;
		_orderflag = $("#orderflag").val();
		_orderflag = _orderflag == null ? "" : _orderflag;
		_saledatefrom = $.trim($('#saledatefrom').val());
		_saledateto = $.trim($('#saledateto').val());
		...
		_deliveryordernumber = $.trim($('#deliveryordernumber').val());
		_salesorderid = $.trim($("#salesorderid").val());
		_salesorderid = _salesorderid == null ? "" : _salesorderid;
		
		_sapsalesorderid = $.trim($("#sapsalesorderid").val());
		_sapsalesorderid = _sapsalesorderid == null ? "" : _sapsalesorderid;
		
		_kunnr = $.trim($("#kunnr").val());
		_kunnr = _kunnr == null ? "" : _kunnr;
		
		_vipid = $.trim($("#vipid").val());
		_vipid = _vipid == null ? "" : _vipid;
		
		_kondm = $.trim($("#kondm").val());
		_kondm = _kondm == null ? "" : _kondm;
		
		_salesclerk = $.trim($("#salesclerk").val());
		_salesclerk = _salesclerk == null ? "" : _salesclerk;
		
		_operator = $.trim($("#operator").val());
		_operator = _operator == null ? "" : _operator;
				window.location.href=basepath + "longhaul/pos/order/orderSystem.ered?reqCode=exportOrder&postType=1&random="
				+ Math.random()
					+"&werks=" + WERKS
					+"&saledatefrom=" + _saledatefrom
					+"&saledateto=" + _saledateto
					+"&salesorderid=" + _salesorderid
					+"&sapsalesorderid=" + _sapsalesorderid
					+"&kunnr=" + _kunnr
					+"&vipid=" + _vipid
					+"&kondm=" + _kondm
					+"&ordertype=" + _ordertype
					+"&orderflag=" + _orderflag
					+"&operator=" + _operator
					+"&salesclerk=" + _salesclerk
					+"&deliveryordernumber=" + _deliveryordernumber
					+"&batchno=" + _batchno
					+"&type=info";
			}
	$("#exportorderinfo").click(function(){
		exportOrder();
	})
	...
	function exportOrderDetail() {
		...
		_ordertype = _ordertype == null ? "" : _ordertype;
		_ordertype = _ordertype =="所有订单类型" ? "" : _ordertype;
		_ordertype = encodeURI(_ordertype);
		_ordertype = encodeURI(_ordertype);
		_orderflag = $("#orderflag").val();
		_orderflag = _orderflag == null ? "" : _orderflag;
		_saledatefrom = $.trim($('#saledatefrom').val());
