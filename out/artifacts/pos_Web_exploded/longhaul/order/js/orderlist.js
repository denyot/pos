$(function() {
	...
	function getorderbypage(page, pageSize) {
		...
		$.getJSON(
				"longhaul/orderSystem.ered?reqCode=orderSystem&postType=1&random="
						+ Math.random(), {
					...
					$.each(data, function(key, val) {
						...
						var _herf = basepath
								+ "/longhaul/orderSystem.ered?reqCode=orderSystemStart&postType=1&salesorderid="
						...
						row = row + "<td>" + val.ordertypetext + "</td>";
						row = row + "<td>" + val.saledate + "</td>";
						vipcard = val.vipcard == null ? "" : val.vipcard;
						row = row + "<td>" + val.customerid + "</td>";
						...
						var selesclerk = val.salesclerk == null || val.salesclerk=="null" ? "" :val.salesclerk;
						row = row + "<td>" + selesclerk + "</td>";
	...
	function getOderType() {
		$.getJSON(
				"longhaul/orderSystem.ered?reqCode=getOrderType&postType=1&werks="
						+ WERKS + "", function(data) {
					var auart = "";
					$("#ordertype")
	...
	$("#del").click(function(event) {
		...
		jConfirm('确认要删除订单吗?', '提示', function(r) {
			if (r == true) {
				...
				$.ajax({
							url : "longhaul/orderSystem.ered?reqCode=del&postType=1",
							cache : false,
							data : {
								orderhead : orderhead
							},
	...
	$("#posting").click(function(event) {
		...
		$.ajax({
					url : "longhaul/orderSystem.ered?reqCode=posting&postType=1",
					cache : false,
					data : {
						orderhead : orderhead
					},
	...
	$("#writeoff").click(function(event) {
		...
		$.ajax({
					url : "longhaul/orderSystem.ered?reqCode=writeoff&postType=1",
					cache : false,
					data : {
						orderhead : orderhead
					},
	...
	function getOrderPageCount() {
		...
		_deliveryordernumber = $.trim($('#deliveryordernumber').val());
		$.post(
				"longhaul/orderSystem.ered?reqCode=getOrderPageCount&postType=1&random="
						+ Math.random(), {
					werks : WERKS.substring(0, WERKS.length - 1),
					saledatefrom : _saledatefrom,
					"saledateto" : _saledateto,
