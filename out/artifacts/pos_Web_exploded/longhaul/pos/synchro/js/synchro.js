$(function() {
	...
	var type=$.getUrlParam("sycn");
	if(type!=null){
		$("#selectRfcNameLaber").hide();
		$("#werksLaber").hide();
	}
...
//				});
			...
			$("#addcxRow").click(function (){
				var myinput = $("<input type='text'/><br/>");
				$("#cxinfo").append(myinput);
			});
			...
			$("#synccx").click(function(){
				...
				if($.trim(text) == ''){
					jAlert("请输入要同步的促销代码！","提示", function(e){
				...
				$.ajax({
						url : "longhaul/pos/synchro/synchroSystem.ered?reqCode=synchroCxdm&option=user&postType=1",
						data : "aktnrs=" + text,
						dataType : 'json',
						success : function(data) {
							dialog.close();
