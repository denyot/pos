		...
		$.ajax({
			...
			success : function(data) {
				if(data == 0){
					...
					var currtDay = new Date();
					firstDay = getTodyay("-").substring(0,7) + "-01";
					currtDay = $('#ordertime').val();
					if(currtDay < firstDay){
						alert('上个月账期已关闭，不允许录单！');
...
//		}
	...
	$("#userstatement").click(function() {
		if ($("#ordertype").val() == "ZOR4" && !checkChangeInfo()) {// 检查换货输入信息
			...
			//积分兑换时检查积分是否足够
			if(ordertype == 'ZYS6'){
				var totalPoint = Number($("#statementtotal").val());
