$(function() {
	...
	function getUserOderType(ordertype) {
		$.getJSON("longhaul/pos/order/orderSystem.ered?reqCode=getOrderType&postType=1&werks=" + WERKS + "&ordertype=" + ordertype, function(data) {
			var auart = "";
			$.each(data, function(key, val) {
	...
	function getsWerksSaleFlag(werks) {
		$.ajax({
			url : "longhaul/pos/order/orderSystem.ered?reqCode=getsWerksSaleFlag&postType=1&werks=" + werks + "",
	...
	function getOrderReason(auart) {
		$.getJSON("longhaul/pos/order/orderSystem.ered?reqCode=getOrderReason&postType=1", {
			auart : auart
		}, function(data) {
			$("#orderreason").empty();
			$.each(data, function(key, val) {
				...
				// if (_ordertype == "ZRE1" || _ordertype == "ZRE2" ||
				// _ordertype == "ZOR4") {
				// if (val.xh == "009") {
				// selected = "selected";
				// }
				// }
				$("#orderreason").append($("<option value=" + val.augru + " " + selected + " >" + val.bezei + "</option>"));
			});
		});
	...
	function getsaleman(noneselectedtext, mode) {
		...
		$.getJSON("longhaul/pos/order/orderSystem.ered?reqCode=getsaleman&postType=1&random=" + Math.random(), {
			...
			$.each(data, function(key, val) {
				...
				// noneselectedtext = mode == "ADD" ? key == 100000 ? val.yyy :
				...
				$("#saleman").append($("<option value=" + val.assistant_name + " " + selected + ">" + val.assistant_name + "</option>"));
			});
		...
		if (event.keyCode == 13) {
	...
	}).autocomplete(
			{
				source : function(request, response) {
					$.ajax({
						url : "longhaul/pos/order/orderSystem.ered?reqCode=getVipRecord&postType=1&option=auto&werks=" + WERKS + "&random="
								+ Math.random() + "",
						dataType : "json",
	...
	function kunnrbyuser() {
		...
		$.ajax({
			url : "longhaul/pos/order/orderSystem.ered?reqCode=getVipRecord&option=user&postType=1&werks=" + WERKS + "&random=" + Math.random() + "",
			dataType : "json",
			data : "kunnr=" + $("#kunnr").val() + "",
			success : function(data) {
				if (data == "") {
	...
	$("#vipid").keydown(function(event) {
		if (ordertype != 'ZJM6' && ordertype != 'ZYS6' && ordertype != 'ZYR2' && ordertype != 'ZJR2' && ordertype != 'ZYS3' && ordertype != 'ZJM3') {
			if ($(this).val() == "")
				$("#registertd").show();
		...
		if (event.keyCode == 13) {
	...
	}).autocomplete(
			{
				source : function(request, response) {
					$.ajax({
						url : "longhaul/pos/order/orderSystem.ered?reqCode=getVipRecord&option=auto&postType=1&werks=" + WERKS + "&random="
								+ Math.random() + "",
						dataType : "json",
						type : "post",
						...
						success : function(data) {
							...
							response($.map(data, function(item) {
								...
								return {
									label : "客户号:" + kunnr + "VIPID:" + vipid + "mingzi:" + hyxm + ""
								};
							}));
						}
					});
		...
		$.ajax({
			...
			success : function(data) {
				...
				$.map(data, function(item) {
					...
					var fax_number = item.fax_number == null ? "" : item.fax_number;
...
//					else
					...
					if (doubleInteval == 1) {
						if (ordertype == "ZJM1" || ordertype == "ZJM2" || ordertype == "ZYS1" || ordertype == "ZYS2") {
							jAlert("<span style='font-size: 16px;color: green;'>" + vipname + "</span>是本月的生日，消费将会获得双倍积分！", "提示", function(e) {
								$("#charg").select();
							});
						}
						...
						if (ordertype == "ZJM1" || ordertype == "ZJM2" || ordertype == "ZYS1" || ordertype == "ZYS2") {
							jAlert("<span style='font-size: 16px;color: green;'>" + vipname + "</span>是本月的结婚纪念日，消费将会获得双倍积分！", "提示", function(e) {
								$("#charg").select();
							});
						}
						doubleIntervalValidate = true;
				...
				if (ordertype == "ZJM3" || ordertype == "ZYS3") {
					...
					$.ajax({
						...
						success : function(data) {
							...
							$.map(data, function(item) {
								$("#matnrSelect").append($("<option value=" + item.choiceorderid + ">" + item.saporderid + "</option>"));
							});
				...
				if (ordertype == "ZJR2" || ordertype == "ZYR2") {
					...
					$.ajax({
						...
						success : function(data) {
							...
							$.map(data, function(item) {
								$("#matnrSelect").append($("<option value=" + item.salesorderid + ">" + item.sapsalesorderid + "</option>"));
							});
				...
				if (ordertype == "ZJM6" || ordertype == "ZYS6") {
					...
					$.ajax({
						...
						success : function(data) {
							if (data == "") {
								jAlert("没有取到兑换品，请检查积分是否足够！", "提示", function(e) {
									$("#vipid").focus();
								});
								return;
							}
							$.map(data,
									function(item) {
										$("#matnrSelect").append(
												$("<option value=" + item.matnr + "-" + item.kbetr + ">" + item.maktx + " 需要积分：" + item.kbetr
														+ "</option>"));
									});
				...
				if(ordertype == "ZJM4" || ordertype == "ZYS4"){
					$("input[type=radio][name=chargtype][value=gift]").attr("checked", "checked");
						...
						$.ajax({
								...
								success : function(data) {
									if (data == "") {
										jAlert("没有取到礼品信息，请检查库存是否足够！", "提示", function(e) {
											$("#vipid").focus();
										});
										return;
									}
									$.map(data,function(item) {
												$("#matnrSelect").append(
														$("<option value=" + item.matnr + "-" + item.labst + ">" + item.maktx + "</option>"));
											});
									$("#matnrSelect").focus();
								}
							});
	...
	$("#charg").keydown(function(event) {
		...
		if (event.keyCode == 13) {
			...
			for ( var i = 0; i < chargDivs.length; i++) {
				if ($.trim($(chargDivs[i]).text()) == $.trim($(this).val())) {
	...
	}).autocomplete(
			{
				source : function(request, response) {
					...
					$.ajax({
						url : "longhaul/pos/order/orderSystem.ered?reqCode=getpcxx&postType=1&ordertype=" + _ordertype + "&option=auto&werks="
								+ WERKS + "&random=" + Math.random() + "",
						dataType : "json",
						data : "charg=" + $.trim(_charg) + "&chargtype=" + _chargtype + "&giftReferencePrice=" + $("#giftReferencePrice").val(),
						success : function(data) {
							if (data == "") {
	...
	function getchargbyuser() {
		...
		$.ajax({
			url : "longhaul/pos/order/orderSystem.ered?reqCode=" + method + "&option=user&ordertype=" + _ordertype + "&postType=1&werks=" + WERKS
					+ "&random=" + Math.random() + "",
			dataType : "json",
			data : "charg=" + $.trim(_charg) + "&chargtype=" + _chargtype + "&giftReferencePrice=" + $("#giftReferencePrice").val() + "&vipid="
					+ $("#vipid").val(),
			success : function(data) {
				if (data == "") {
					var chargalertstr = "输入批次不存在!";
				...
				$.map(data, function(item) {
					...
					var oldsapsaleorderid = item.oldsapsaleorderid == null ? "" : item.oldsapsaleorderid;
					var oldsalesorderid = item.oldsalesorderid == null ? "" : item.oldsalesorderid;
					var oldunionpay = item.oldunionpay == null ? 0 : item.oldunionpay;
					...
					oldcurrentInteval = item.oldcurrentintegral;
	...
	function chosecharginfo(item, type) {
		...
		if (charginfo.lgort != '0001' && $('input:radio[name="chargtype"]:checked').val() == "charg") {
		...
		if (charginfo.extwg == 'EA' || charginfo.extwg == 'EH' || charginfo.extwg == 'ES' || charginfo.extwg == 'EW') {
			$("#doubleorsingleSpan").show();
		}
		myOldRealPrice = charginfo.realprice;
		// alert(myOldRealPrice);

		// if(charginfo.labst < 1){
		...
		if ((charginfo.labst == null || charginfo.labst <= 0) && !(ordertype == 'ZJM2' && $("#swaptype").val() == 'ZIN')
				&& !(ordertype == 'ZYS2' && $("#swaptype").val() == 'ZIN') && ordertype != 'ZJR1' && ordertype != 'ZYR1' && ordertype != 'ZYR2'
				&& ordertype != 'ZJR2' && ordertype != 'ZJM3' && ordertype != 'ZYS3') {
			...
			// _pcimage = _pcimage == null || _pcimage == "" ? "zjzb.gif" :
			// _pcimage;
			...
			// jAlert("客户积分不够兑换商品!当前客户积分:" + _kunnrjf + "商品所需积分:" + _giftjf,
			// '提示', function(r) {
			// });
			// return false;
			// }
			...
			// if (usedorderid != "") {
			// jAlert($("#charg").val() + "已存在于订单" + usedorderid + "不能再次加入订单!",
			// '提示', function(r) {
			// clearcharginfo();
			...
			// setpricebychargtype(_price, _realprice);
			// }
			// if ((_ordertype == "ZYS2" && ($("#swaptype").val() ==
			// "ZIN"||$("#swaptype").val() == "GZIN") && _chargtype == "charg")
			// || (_ordertype == "ZYS2" && $("#swaptype").val() == "ZIN1" &&
			// _chargtype == "charg")
			// || (_ordertype == "ZJM2" && ($("#swaptype").val() ==
			// "ZIN"||$("#swaptype").val() == "GZIN") && _chargtype == "charg")
			// || (_ordertype == "ZJM2" && $("#swaptype").val() == "ZIN1" &&
			// _chargtype == "charg")
			// || (_ordertype == "ZYR1" && _chargtype == "charg") || (_ordertype
			// == "ZJR1" && _chargtype == "charg")) {
			// $("#number").removeClass("inputkey");
			// $("#discount1").val(100);
			// $("#discount2").val(100);
			// // $("#discount3").val(100);
			...
			// $("#number").removeClass("inputkey");
			// $("#logrt").hide();
			// if ($("#ordertype").val() == 'ZYS1' || ($("#ordertype").val() ==
			// 'ZYS2' && $("#swaptype").val() == 'ZOUT') ||
			// $("#ordertype").val() == 'ZYS4') {
			// $.ajax({
			// url :
			// "longhaul/pos/order/orderSystem.ered?reqCode=getDiscount&option=user&postType=1&charg="
			// + $("#charg").val() + "&kunnr=" + $("#kunnr").val() + "&matnr=" +
			// $("#matnr").val(),
			// dataType : 'json',
			// success : function(data) {
			...
			// "ZJM2")&&($("#swaptype").val()=="ZIN"||$("#swaptype").val()=="GZIN"))||(ordertype
			// == "ZYR1")||(ordertype == "ZJR1")){
			// if((ordertype == "ZYS2"||ordertype ==
...
//					alert("ok");
			...
			if (charginfo.ckxx == '0') {
				...
				if (matkl == 'D' || matkl == 'G' || matkl == 'P' || matkl == 'S') {
					...
					$("#ztjtj").val("0");
					if ((ordertype == 'ZJM2' || ordertype == 'ZYS2') && $("#swaptype").val() == 'ZIN') {
						if (matkl == 'P') {
							$("#personcost").val(30);
						} else if (matkl == 'G') {
							$("#personcost").val(20);
			...
			if ((ordertype == "ZJM2" && $("#swaptype").val() != "ZOUT") || (ordertype == "ZYS2" && $("#swaptype").val() != "ZOUT")
					|| ordertype == "ZJR1" || ordertype == "ZYR1") {
				if (charginfo.oldvipcard != $("#vipid").val()) {
					jAlert("该批次不是该会员消费的，不能做退换货处理！", "提示", function(e) {
						$("#charg").focus();
					});
					return;
				}
				...
				$("#realprice").val(Number(charginfo.realprice) - Number($("#personcost").val()) * Number(charginfo.oldgoldWeight));
			...
			if ($("#personcostlabel").css("display") != 'none') {
				...
				if ((_ordertype == "ZYS2" && ($("#swaptype").val() == "ZIN" || $("#swaptype").val() == "GZIN") && _chargtype == "charg")
						|| (_ordertype == "ZYS2" && $("#swaptype").val() == "ZIN1" && _chargtype == "charg")
						|| (_ordertype == "ZJM2" && ($("#swaptype").val() == "ZIN" || $("#swaptype").val() == "GZIN") && _chargtype == "charg")
						|| (_ordertype == "ZJM2" && $("#swaptype").val() == "ZIN1" && _chargtype == "charg")
						|| (_ordertype == "ZYR1" && _chargtype == "charg") || (_ordertype == "ZJR1" && _chargtype == "charg")) {
					$("#number").removeClass("inputkey");
					// $("#discount1").val(100);
					// $("#discount2").val(100);
					// $("#discount3").val(100);
					...
					if ($("#ordertype").val() == 'ZYS1' || ($("#ordertype").val() == 'ZYS2' && $("#swaptype").val() == 'ZOUT')
							|| $("#ordertype").val() == 'ZYS4') {
						$.ajax({
							url : "longhaul/pos/order/orderSystem.ered?reqCode=getDiscount&option=user&postType=1&charg="+$("#charg").val() 
									+ "&kunnr=" + $("#kunnr").val() + "&matnr=" + $("#matnr").val(),
							dataType : 'json',
							success : function(data) {
								$("#discount1").val(data.discount1);
								$("#discount2").val(data.discount3);
				...
				if (((ordertype == "ZYS2" || ordertype == "ZJM2") && ($("#swaptype").val() == "ZIN" || $("#swaptype").val() == "GZIN"))
...
//					}
			...
			function(event) {
				if (event.keyCode == 13) {
					if ($("#showmatnrinfo").css("display") == "none") {
						...
						// _price = (Number(_goldvalue) * _goldweight) +
						// Number(_personcost);
						// _realprice = $("#realprice").val() *
						// Number($("#number").val()); // 实际价格
						// _price = _price.toFixed(0);
						// //setpricebychargtype(_price, _realprice);
						// } else {
						// _price = $("#ztjtj").val() *
						// Number($("#number").val()); // 价格=标签价格*数量
						// _realprice = $("#realprice").val() *
						// Number($("#number").val()); // 实际价格
						// _price = _price.toFixed(0);
						// //setpricebychargtype(_price, _realprice);
						...
						// if ((_ordertype == "ZYS2" && ($("#swaptype").val() ==
						// "ZIN"||$("#swaptype").val() == "GZIN") && _chargtype
						// == "charg") || (_ordertype == "ZYR1" && _chargtype ==
						// "charg")) {
						// $("#number").removeClass("inputkey");
						// $("#discount1").val(100);
						// $("#discount2").val(100);
				...
				if (event.keyCode == 13) {
					...
					if ($("#showmatnrinfo").css("display") == "none") {
						...
						if ($("#swaptype").val() == "ZIN" || _ordertype == "ZRE1") { // 如果换入需要输入库位,换出不需要库位
							...
							if ((chargtype == "charg" && this.value != "" && (this.value == "0001" || this.value == "0002" || this.value == "0003"
									|| this.value == "0004" || this.value == "0006" || this.value == "0008"))
			...
			function(e) {
				if (e.keyCode != 13) {
					if (ordertype == "ZYR1" || ordertype == "ZJR1"
							|| ((ordertype == "ZYS2" || ordertype == "ZJM2") && ($("#swaptype").val() == "ZIN" || $("#swaptype").val() == "GZIN"))) {
	...
	$("#realprice").keydown(
			function(event) {
				if (event.keyCode == 13) {
					...
					if (ordertype == "ZYR1" || ordertype == "ZJR1" || (ordertype == "ZYS2" && (swaptype == "ZIN" || swaptype == 'ZIN1'))
					...
					if ($('input:radio[name="chargtype"]:checked').val() !== "gift") {
						if (!checkNum(this)) {
						...
						} else if ($("#realprice").val() == 0 && $("#giftMethod").val() == '') {
							...
							$.ajax({
								...
								success : function(retdata) {
									...
									if ($("#realprice").data("kosrt") != null && $("#realprice").data("kosrt") != "") {
										jConfirm("该商品为有组合商品，是否带出组合商品信息！", "提示", function(e) {
											if (e) {
												getGroupGoodsInfo($("#realprice").data("kosrt"));
											} else {
												btnAddRow();
											}
										});
	...
	function btnAddRow() {
		...
		var depreciationPrice = $("#depreciationPrice").val() == null ? '' : $("#depreciationPrice").val();
		var giftMethodStr = ""
		switch (giftMethod) {
		case '01':
			giftMethodStr = "赠链";
		...
		if (chargtype == "charg") {
			...
			if ($("#allGift").attr("checked") == 'checked' || ordertype == "ZYS4" || ordertype == "ZJM4") {
				...
				allGiftNumber++;
			} else if ($("#inputPackage").attr("checked") == 'checked') {
				style = "tdbclevel";
				upnumbertext = "00";
				upnumber = Number(toplevelnumber) * 10;
				toplevelnumber = toplevelnumber + 1;
				...
				// style = "tdtoplevel";
				upnumbertext = "00";
				toplevelstr = "<div id=levelbc" + upnumber + " style='display:none'>" + upnumber + "</div>"; // 增加礼品时记录礼品行号包括有多少礼品
				...
				if (_ordertype == "ZRE2" || _ordertype == "ZKL" || _ordertype == "ZJM6" || _ordertype == "ZYS6") { // 赠品的特殊处理
		...
		var logrt = "<div id=logrt" + rownum + ">" + $("#logrtinput").val() + "</div>";
		...
		_goldweight = (Number($("#goldweight").val()) * doubleorsingle).toFixed(3);
		var _personcost = (Number($("#personcost").val()) * _goldweight).toFixed(0)
		var personcost = "<div id=personcost" + rownum + ">" + _personcost + "</div>"; // 人工费用
		...
		case "ZYS2": // 换货订单类型
			if (chargtype == "charg") {
				...
				logrt = "<div id=logrt" + rownum + ">" + $("#logrtinput").val() + "</div>";
				if ($("#swaptype").val() == "ZIN" || $("#swaptype").val() == "GZIN") {
		...
		if (chargtype == "charg" && _ordertype != "ZOR8") {
			...
			_tempmatnr = $("#matnr").val().indexOf("-") != -1 ? $("#matnr").val().substring(0, $("#matnr").val().indexOf("-")) : $("#matnr").val();
		} else {
			_tempcharg = "";
			_tempmatnr = $("#charg").val();
		}

		if (ordertype == "ZJM6" || ordertype == "ZYS6" || ordertype == 'ZYS4' || ordertype == 'ZJM4') {
			_tempmatnr = _tempmatnr.substring(0, _tempmatnr.indexOf("-"));
		}
		
		var charg = "<div id=charg" + rownum + ">" + (isGoldBar ? "" : _tempcharg.toUpperCase()) + "&nbsp;</div>"; // 批次
		var matnr = "<div id=matnr" + rownum + ">" + _tempmatnr.toUpperCase() + "&nbsp; </div>"; // 物料

		var needNEDiv = "<div id=needNE" + rownum + " style='display:none;'>" + ifneedNe + "</div>"; // 是否需要赠链
		var giftMethodDiv = "<div id=giftMethod" + rownum + " style='display:none;'>" + giftMethod + "</div>"; // 赠送方式
		var depreciationPriceDiv = "<div id=depreciationPrice" + rownum + " style='display:none;'>" + depreciationPrice + "</div>"; // 赠送方式

		var zhlhx = "<div id=zhlhx" + rownum + " >" + $("#zhlhx").val() + "&nbsp; </div>"; // 商品ID
		var uniteprice = "<div id=uniteprice" + rownum + ">" + Number($("#ztjtj").val()).toFixed(0) + "&nbsp;</div>"; // 单价
		var number = "<div id=number" + rownum + ">" + _number + "</div>"; // 数量
		var goldweight = "<div id=goldweight" + rownum + ">" + _goldweight + "</div>"; // 金重
		var goldvalue = "<div id=goldvalue" + rownum + " style='display:none'>" + (isGoldBar ? $("#realitygoldvalue").val() : $("#goldvalue").val()) + "</div>"; // 金价

		var goldrealprice = "<div id=goldrealprice" + rownum + ">"
				+ (Number($("#personcost").val()) + Number($("#goldweight").val()) * Number($("#goldvalue").val())).toFixed(0) + "&nbsp;</div>"; // 金价初始总价
		...
		var salepromotion = "<div id=salepromotion" + rownum + ">" + (($("#discount12").get(0).selectedIndex == 1) ? $("#salepromotion").val() : "")
				+ "</div>"; // 促销代码
		_realDiscount =(Number(_ztjtj)!=0 ? Number(_realprice)/Number(_ztjtj) * 100 : 100).toFixed(0);
		var discount1 = "";
		var discount2 = "";
		var discount3 = "";
		var discount4 = "";
		...
		if (((ordertype == 'ZJM2' || ordertype == 'ZYS2') && ($("#swaptype").val() == 'GZIN' || $("#swaptype").val() == 'ZIN')) || ordertype == 'ZJR1' || ordertype == 'ZYR1') {
			discount1 = "N/A";
			discount2 = "N/A";
			discount3 = "N/A";
			discount4 = "N/A";
		...
		discount6 = "<div id=discount6" + rownum + ">" + _realDiscount + "</div>"; // 折扣6

		
		
		var marketprivilege = "<div id=marketprivilege" + rownum + ">" + _marketprivilege + "</div>";
		var selfprivilege = "<div id=selfprivilege" + rownum + ">" + _selfprivilege + "</div>";
		var marketticketprice = "<div id=marketticketprice" + rownum + ">" + _marketticketprice + "</div>";
		var selfticketprice = "<div id=selfticketprice" + rownum + ">" + _selfticketprice + "</div>";
		var vipintegralNum = Number($("#vipintegral").val());
		var vipintegral = "<div id=vipintegral" + rownum + ">" + _vipintegral + "</div>";
		var currentIntegralNum = (Number($("#currentIntegral").val()) == 0 ? 0 : (Number(_realprice) / Number($("#currentIntegral").val())))
				.toFixed(0);
		...
		if ((ordertype == 'ZYS2' && ($("#swaptype").val() == "ZIN" || $("#swaptype").val() == "GZIN"))
				|| (ordertype == 'ZJM2' && ($("#swaptype").val() == "ZIN" || $("#swaptype").val() == "GZIN")) || (ordertype == 'ZJR1')
				|| (ordertype == 'ZYR1') || (ordertype == 'ZJM3') || (ordertype == 'ZJM8') || (ordertype == 'ZJR2') || (ordertype == 'ZJR3')
				|| (ordertype == 'ZYR2') || (ordertype == 'ZYR3') || (ordertype == 'ZYS3') || (ordertype == 'ZYS8')) {
		...
		var currentIntegral = "<div id=currentIntegral" + rownum + ">" + (currentIntegralNum) + "</div>";
		...
		var settlegoldvalue = "<div id=settlegoldvalue" + rownum + ">" + (settlegoldvalueNum) + "</div>";
		var realitygoldvalue = "<div id=realitygoldvalue" + rownum + ">" + (realitygoldvalueNum) + "</div>";
				
		// 设置合计

		$("#totalprice").text((Number($("#totalztjtj").text()) + Number($("#ztjtj").val())).toFixed(2)); // 总的标签价格
		$("#totalnumber").text((Number($("#totalnumber").text()) + Number(_number))); // 总数量
		$("#toalpersoncost").text((Number($("#toalpersoncost").text()) + Number(_personcost)).toFixed(2)); // 人工费用
		...
		row = row + "<td class=" + style + ">" + zhlhx + needNEDiv + giftMethodDiv + depreciationPriceDiv + "</td>";
		row = row + "<td class=" + style + " align='center'>" + pcimage + "</td>";
		row = row + "<td style='display:none' class=" + style + " align='right'>" + uniteprice + "</td>";
		row = row + "<td style='display:none' class=" + style + ">" + currentIntegral + "</td>";
		row = row + "<td class=" + style + " align='right'>" + number + "</td>";
		goodpersoncosttdshow = $("#thpersoncost").css("display") == "none" ? "display:none" : "";
		goodtdshow = $("#thgoldweight").css("display") == "none" ? "display:none" : "";
		...
		row = row + "<td  id='personcosttd" + rownum + "'  class=" + style + "  align='right' style=" + goodpersoncosttdshow + ">" + personcost
				+ "</td>";
		...
		row = row + "<td class=" + style + ">" + ordertItemype + "" + ordertItemypeshow + "<br/>" + giftMethodStr + "</td>";

		if (ordertype != "ZJM6" && ordertype != "ZYS6" && ordertype != "ZOR6" && (!isGoldBar) && ordertype != "ZJM4" && ordertype != "ZYS4" ) {
			...
			row = row + "<td class=" + style + ">" + marketticketprice + "</td>";
			row = row + "<td class=" + style + ">" + selfticketprice + "</td>";
			row = row + "<td class=" + style + ">" + vipintegral + "</td>";
			row = row + "<td class=" + style + " style='display:none;'>" + salepromotion + "</td>";
			// row = row + "<td class=" + style + ">" + cashcoupon + "</td>";

			row = row + "<td class=" + style + " id='logrttd" + rownum + "' >" + logrt + "</td>";
		}else{
			if( ordertype != "ZJM4" && ordertype != "ZYS4" ){
				row = row + "<td class=" + style + ">" + settlegoldvalue + "</td>";
				row = row + "<td class=" + style + ">" + realitygoldvalue + "</td>";
			}
			row = row + "<td class=" + style + " id='logrttd" + rownum + "' >" + logrt + "</td>";
	...
	function creatTableRow(salesorderid) {
		$.getJSON("longhaul/pos/order/orderSystem.ered?reqCode=getOrderItem&postType=1", {
			...
			$.each(data, function(key, val) {
				...
				if (val.orderitemtype == 'ZTNN' && (val.salesorderitem % 10) == 0) {
				...
				posnrnumber = "<div  id=posnrnumber" + key + "> " + val.salesorderitem + " </div>" + "<div id=upnumber" + _chargtype + key
						+ "  style='display:none'>" + val.salesorderitem + "</div>" + toplevelstr + "";
				...
				_zhlhx = "<div id=zhlhx" + key + " >" + goodname + "</div>"; // 商品ID
				_uniteprice = "<div id=uniteprice" + key + ">" + Number(val.tagprice).toFixed(0) + "&nbsp;</div>"; // 单价
				_number = "<div id=number" + key + ">" + val.salesquantity + "</div>"; // 数量
				_personcost = "<div id=personcost" + key + ">" + Number(val.goodsprocessingfee).toFixed(0) + "</div>"; // 人工费用
				_goldweight = "<div id=goldweight" + key + ">" + Number(val.goldweight).toFixed(3) + "</div>"; // 金重
				_goldvalue = "<div id=goldvalue" + key + " style='display:none'>" + val.goldprice + "</div>"; // 金价
				_receivable = "<div id=receivable" + key + " >" + Number(val.totalamount).toFixed(0) + "</div>"; // 合计
				
				_realDiscount =parseInt(Number(val.tagprice)!=0 ? Number(val.totalamount)/Number(val.tagprice) *100 : 100);
				
				if(Number(val.goldprice) > 0){
					_realDiscount = parseInt(Number(val.totalamount)/(Number(val.goldprice)*Number(val.goldweight)+Number(val.goodsprocessingfee))*100);
					_realDiscount = _realDiscount > 100 ? 100 : _realDiscount;
				}
				
				
				//alert(_realDiscount);
				_discount1 = "<div id=discount1" + key + ">" + (val.discount1 == null ? "" : val.discount1) + "</div>"; // 折扣1
				_discount2 = "<div id=discount2" + key + ">" + (val.discount2 == null ? "" : val.discount2) + "</div>"; // 折扣2
				_discount3 = "<div id=discount3" + key + ">" + (val.discount3 == null ? "" : val.discount3) + "</div>"; // 折扣3
				_discount4 = "<div id=discount4" + key + ">" + (val.discount4 == null ? "" : val.discount4) + "</div>"; // 折扣4
				_discount5 = "<div id=discount5" + key + ">" + (val.discount5 == null ? "" : val.discount5) + "</div>"; // 折扣5
				_discount6 = "<div id=discount5" + key + ">" + (_realDiscount) + "</div>"; // 折扣6

				_marketprivilege = "<div id=marketprivilege" + key + ">" + (val.marketprivilege == null ? "" : val.marketprivilege) + "</div>"; // 商场优惠券
				_selfticketprice = "<div id=selfticketprice" + key + ">" + (val.selfticketprice == null ? "" : val.selfticketprice) + "</div>"; // 自发优惠券
				_vipintegral = "<div id=vipintegral" + key + ">" + (val.vipintegral == null ? "" : val.vipintegral) + "</div>"; // 商场现金券
				_selfprivilege = "<div id=selfprivilege" + key + ">" + (val.selfprivilege == null ? "" : val.selfprivilege) + "</div>"; // 自发现金券
				_marketticketprice = "<div id=marketticketprice" + key + ">" + (val.marketticketprice == null ? "" : val.marketticketprice)
						+ "</div>"; // VIP抵现优惠

				_ztjtj = "<div id=ztjtj" + key + " >"
						+ (Number(val.goodsprocessingfee) == 0 && val.salesorderitem % 10 == 0 && _ordertype != 'ZKL' ? Number(val.tagprice).toFixed(
								2) : '0.00') + "</div>"; // 标签价格
				_realTagPrice = "<div id=realTagPrice" + key + " >" + Number(val.tagprice).toFixed(0) + "</div>"; // 实际标签价格

				_realprice = "<div id=realprice" + key + ">"
						+ ((ordertype == "ZYR2" || ordertype == "ZJR2") ? "-" + Number(val.netprice).toFixed(0) : Number(val.netprice).toFixed(0))
						+ "</div>"; // 实销价格
				...
				_productpictureurl = val.productpictureurl == null || val.productpictureurl == "" ? (compImage = "CHJ" ? "CHJ-LOGO.JPG" : "vt.jpg")
						: val.productpictureurl;
				var imgpath ="sappic/" + _productpictureurl;
				imaghref = "<a href=" + imgpath + " class='tooltip'>";
				_pcimage = "<div id=pcimage" + key + ">" + imaghref + "<img id=pcimagesrc   src=" + imgpath + " height='40' width='38' alt='"
						+ _productpictureurl + "' onerror='this.src=\"longhaul\/pos\/order\/images\/zjzb.gif\"'/></a></div>";
				...
				setTotalObjectByName('totalrealprice', ((ordertype == "ZYR2" || ordertype == "ZJR2") ? -val.netprice : val.netprice), 2);
				var row = "<tr><td class=" + style + ">" + row + "</td>";
				if (opmode == "view") {
				...
				row = row + "<td class=" + style + "  id='personcosttd" + key + "'  align='right' style='" + goodtdshow + "'>" + _personcost
						+ "</td>";
				row = row + "<td class=" + style + "   id='goldtr" + key + "'   align='right'style='" + goodtdshow + "'>" + _goldweight + _goldvalue
						+ "</td>";
				row = row + "<td class=" + style + " style='display:none'  align='right'>" + _receivable + "</td>";

				if (orderTypeCheck() || ordertype == 'ZYS6' || ordertype == 'ZJM6' ) {
					row = row + "<td class=" + style + " align='right'>" + _ztjtj + "</td>";
					row = row + "<td  style='display:none' class=" + style + " align='right'>" + _realTagPrice + "</td>";
				}
				if(ordertype == "ZYS3" || ordertype == "ZJM3" || ordertype == "ZYS8" || ordertype == "ZJM8"){
					row = row + "<td class=" + style + " align='right'>" + _ztjtj + "</td>";
					row = row + "<td  style='display:none' class=" + style + " align='right'>" + _realTagPrice + "</td>";
				}
				...
				row = row + "<td class=" + style + ">" + ordertItemype + "" + ordertItemypeshow + "<br/>" + giftMethodStr + "</td>";
				if (orderTypeCheck() ) {
					row = row + "<td class=" + style + ">" + _discount1 + "</td>";
					row = row + "<td class=" + style + ">" + _discount2 + "</td>";
					row = row + "<td class=" + style + ">" + _discount5 + "</td>";
					row = row + "<td class=" + style + ">" + _discount3 + "</td>";
					row = row + "<td class=" + style + ">" + _discount4 + "</td>";
					row = row + "<td class=" + style + ">" + _discount6 + "</td>";

					row = row + "<td class=" + style + ">" + _marketprivilege + "</td>";
					row = row + "<td class=" + style + ">" + _selfticketprice + "</td>";
					row = row + "<td class=" + style + ">" + _vipintegral + "</td>";
					row = row + "<td class=" + style + ">" + _selfprivilege + "</td>";
					row = row + "<td class=" + style + ">" + _marketticketprice + "</td>";
				}
	...
	function creatTablehead(salesorderid) {
		...
		$.getJSON("longhaul/pos/order/orderSystem.ered?reqCode=getOrderhead&postType=1", {
			...
			$.each(data, function(key, val) {
				...
				if (val.storereceipt != null && val.storereceipt != '') {
				...
				$("#currentIntevalSpan").text(val.currentintegral);
				$("#currentIntevalSpanShow").text(val.currentintegral);
	...
	$("#tablecontent tr th").dblclick(function() {
		...
		if(ordertype == 'ZYS3' || ordertype == 'ZYR2' || ordertype == 'ZJM3'  || ordertype == 'ZJR2' || ordertype == 'ZJR1' 
			|| ordertype == 'ZYR1' || ordertype == 'ZYS4' || ordertype == 'ZJM4'){
			return;
		}
			...
			success : function(retdata) {
				...
				if (retdata.length == 0) {
					jAlert("没有批次对应的礼品！", "提示", function(e) {
						...
						// $("#matnrlabel").hide();
						// inputcharg =
						// $(thistr).find("div[id^='matnr']").text() + "物料号";
						// }
						// log("录赠品在" + inputcharg + "");
						//
				...
				$.map(retdata, function(item) {
					...
					$("#charg").hide();
					$("#matnrSelect").show();
					$("#matnrSelect").append("<option value='" + item.matnr + "'>" + item.name + "</option>");
				});
			}
		});
		...
		jConfirm("确定删除吗？", "请确认？", function(e) {
			if (e) {
				...
				if (_ordertype == "ZRE2" || _ordertype == "ZKL" ) {
					...
					$("input[type=radio][name=chargtype][value=gift]").attr("checked", "checked");
				} else if ( _ordertype == "ZJM4" || _ordertype == "ZYS4"){
					 $("#charglabel").html("赠品料号:");
				}else {
					$("input[type=radio][name=chargtype][value=charg]").attr("checked", "checked");
					// $("#charglabel").html("批&nbsp;&nbsp;&nbsp;&nbsp;次: ");
				...
				if (_ordertype == "ZJM6" || _ordertype == "ZYS6" || _ordertype == "ZJM4" || _ordertype == "ZYS4" ) {
				...
				if (trlength <= 2 || _ordertype == "ZJM1" || _ordertype == "ZYS1") {
	...
	function getgiftjf(charg) {
		$.ajax({
			async : false,
			url : "longhaul/pos/order/orderSystem.ered?reqCode=getGiftJF&postType=1&charg=" + charg + "&random=" + Math.random() + "",
			success : function(data) {
				$("#giftjf").text(data);
			}
		});
	...
	function getgoldprices(chargtype) {
		$.getJSON("longhaul/pos/order/orderSystem.ered?reqCode=getGoldPrices&postType=1", {
			chargtype : chargtype,
			random : Math.random()
		}, function(data) {
			if (data == null || data == "") {
	...
	function changerealprice(thistr, thisvalue, oldtext) {
		$("#totalrealprice").text((Number($("#totalrealprice").text()) + (thisvalue - oldtext)).toFixed(0));
		$("#total").text((Number($("#total").text()) + (thisvalue - oldtext)).toFixed(0));
			...
			if (keycode == 13 && inputIns.unbind("blur")) {
				if (checkNum(this)) {
					...
					if (_goldvalue != 0) {
						...
						$("#toalpersoncost").text(Number(_totalpersoncost).toFixed(0));
					} else {
	...
	function getordertableinfo() {
		...
		if (ordertype == 'ZJM3' || ordertype == 'ZJM8' || ordertype == 'ZYS3' || ordertype == 'ZYS8') {
			orderhead = orderhead + "\"choiceorderid\":\"" + $.trim(matnrSelectId) + "\",";
		} else if (ordertype == 'ZJR2' || ordertype == 'ZJR3' || ordertype == 'ZYR2' || ordertype == 'ZYR3') {
			orderhead = orderhead + "\"oldsaleorderid\":\"" + $.trim(matnrSelectId) + "\",";
		}
		...
		if (ordertype == 'ZJM3' || ordertype == 'ZJM8' || ordertype == 'ZYS3' || ordertype == 'ZYS8') {
			_totalrealprice = $("#choiceOrderTotalMoney").val();
		}else{
		...
		orderhead = orderhead + "\"currentIntegral\":" + $.trim(_currentInteval) + ",";
		_cash = $("#cash").val();
		_cash = $.trim(_cash);
		_cash = _cash == "" ? 0 : _cash;
		...
		$("#tablecontent tr").each(function() { // 得到每一个tr
			...
			_giftMethod = earchobject.find("div[id^='giftMethod']").text(); // 赠送方式
			_depreciationPrice = earchobject.find("div[id^='depreciationPrice']").text(); // 折旧费
			...
			_settlegoldvalue = earchobject.find("div[id^='settlegoldvalue']").text();
			_realitygoldvalue = earchobject.find("div[id^='realitygoldvalue']").text();
			...
			_discount1 = earchobject.find("div[id^='discount1']").text();
			_discount2 = earchobject.find("div[id^='discount2']").text();
			_discount3 = earchobject.find("div[id^='discount3']").text();
			_discount4 = earchobject.find("div[id^='discount4']").text();
			_discount5 = earchobject.find("div[id^='discount5']").text();
			_ordertItemype = earchobject.find("div[id^='ordertItemype']").text()
			_ordertItemype = (_ordertype == 'ZYS8' || _ordertype == 'ZJM8') ? 'ZL3N' : _ordertItemype;
			_ordertItemype = (_ordertype == 'ZYR3' || _ordertype == 'ZJR3') ? 'ZG3N' : _ordertItemype;
			...
			// 自发券优惠
			_selfprivilege = earchobject.find("div[id^='selfprivilege']").text();
			_selfprivilege = $.trim(_selfprivilege);
			_selfprivilege = _selfprivilege == "" ? 0 : _selfprivilege;

			// 商场优惠券
			_marketprivilege = earchobject.find("div[id^='marketprivilege']").text();
			_marketprivilege = $.trim(_marketprivilege);
			_marketprivilege = _marketprivilege == "" ? 0 : _marketprivilege;

			// VIP抵现优惠
			_vipintegral = earchobject.find("div[id^='vipintegral']").text();
			_vipintegral = $.trim(_vipintegral);
			_vipintegral = _vipintegral == "" ? 0 : _vipintegral;

			// 商场现金券
			_marketticketprice = earchobject.find("div[id^='marketticketprice']").text();
			_marketticketprice = $.trim(_marketticketprice);
			_marketticketprice = _marketticketprice == "" ? 0 : _marketticketprice;

			// 自发现金券
			_selfticketprice = earchobject.find("div[id^='selfticketprice']").text();
			_selfticketprice = $.trim(_selfticketprice);
			_selfticketprice = _selfticketprice == "" ? 0 : _selfticketprice;
			...
			_salepromotion = earchobject.find("div[id^='salepromotion']").text();
			_salepromotion1 = earchobject.find("div[id^='salepromotion']").text();
			// alert(_salepromotion);
			...
			tablestr = tablestr + "\"giftMethod\":\"" + $.trim(_giftMethod) + "\",";
			tablestr = tablestr + "\"depreciationPrice\":\"" + $.trim(_depreciationPrice) + "\",";
			...
			tablestr = tablestr + "\"settlegoldvalue\":\"" + $.trim(_settlegoldvalue) + "\",";
			tablestr = tablestr + "\"realitygoldvalue\":\"" + $.trim(_realitygoldvalue) + "\",";
			...
			tablestr = tablestr + "\"discount1\":\"" + $.trim(_discount1) + "\",";
			tablestr = tablestr + "\"discount2\":\"" + $.trim(_discount2) + "\",";
			tablestr = tablestr + "\"discount3\":\"" + $.trim(_discount3) + "\",";
			tablestr = tablestr + "\"discount4\":\"" + $.trim(_discount4) + "\",";
			tablestr = tablestr + "\"discount5\":\"" + $.trim(_discount5) + "\",";
			...
			tablestr = tablestr + "\"selfprivilege\":\"" + _selfprivilege + "\",";
			tablestr = tablestr + "\"marketprivilege\":\"" + _marketprivilege + "\",";
			tablestr = tablestr + "\"vipintegral\":\"" + _vipintegral + "\",";
			tablestr = tablestr + "\"marketticketprice\":\"" + _marketticketprice + "\",";
			tablestr = tablestr + "\"selfticketprice\":\"" + _selfticketprice + "\",";

			tablestr = tablestr + "\"thankIntegral\":\"" + _thankIntegral + "\",";
			tablestr = tablestr + "\"currentIntegral\":\"" + _currentIntegral + "\",";
			tablestr = tablestr + "\"salepromotion\":\"" + _salepromotion + "\",";
				...
				buttons : {
					"付款" : function() {
						if (opmode == "view" && useroption == "print") {
							_salesorderid = $("#salesorderid").val();
							window.showModalDialog(basepath + "longhaul/pos/order/dm_dyxp.jsp?ywxh=" + _salesorderid + "&dwbh=" + WERKS,
									'printWindow', 'dialogWidth=650px;dialogHeight=700px');
							$('input').attr('disabled', true);
							$('select').attr('disabled', true);
							return;
						}
								...
								if (retdata.sapsalesorderid == null || retdata.sapsalesorderid == "") { // 如果SAP订单号为空
									if (retdata.salesorderid != null || retdata.salesorderid != "") { // 如果SAP没有保存成功,但是AIG保存成功
										if (useroption == "print") {
											...
											// resizable=yes');
											window.showModalDialog(basepath + "longhaul/pos/order/dm_dyxp.jsp?ywxh=" + retdata.salesorderid
													+ "&dwbh=" + WERKS, 'printWindow', 'dialogWidth=650px;dialogHeight=700px');
											$('input').attr('disabled', true);
											$('select').attr('disabled', true);
										} else {
											var messagestr = "";
									...
									if (useroption == "print") {
										...
										// 'top=10,width=650,height=800,scrollbars=yes,
										// resizable=yes');
										window.showModalDialog(basepath + "longhaul/pos/order/dm_dyxp.jsp?ywxh=" + retdata.salesorderid + "&dwbh="
												+ WERKS, 'printWindow', 'dialogWidth=650px;dialogHeight=700px');
									} else {
					...
					function() {
						if ($("#ordertype").val() == "ZOR4" && !checkChangeInfo()) {// 检查换货输入信息
							...
							if (ordertype == 'ZYS3' || ordertype == 'ZJM3') {
								...
								$("#statementtotal").val($("#totalrealprice").text());
								_usercash = Number($("#totalrealprice").text()) - $("#unionpay").val() - $("#shoppingcard").val()
										- $("#selfprivilege").val() - $("#marketprivilege").val() - $("#vipintegral").val();
								$("#cash").val(_usercash);
							}
							_unionpay = $("#unionpay").val();
							_shoppingcard = $("#shoppingcard").val();
							...
							if (trlength == 2) {
								...
								if (needNECount > 0) {
									jAlert("你有" + needNECount + "个含赠链吊坠需要输入赠链,请输入赠链信息后再提交！", "提示", function(e) {
										$("#charg").focus();
									});
									return;
								} else if (needNECount < 0) {
									jAlert("你有录入" + Math.abs(needNECount) + "条赠链信息，但是没有发现需要配赠链的吊坠信息，请检查！", "提示", function(e) {
										$("#charg").focus();
									});
									return;
								}
								...
								if (ordertype == "ZOR6") {// || ordertype ==
									...
									if (salemanconf == 0) {
										...
										if ((ordertype != "ZJM3" && ordertype != "ZYS3" && ordertype != "ZJR2" && ordertype != "ZYR2"
												&& ordertype != "ZJM6" && ordertype != "ZYS6" && ordertype != 'ZJM4' && ordertype != 'ZYS4')) {
	...
	$("#orderprint").click(
			function() {
				...
				window.showModalDialog(basepath + "/longhaul/pos/order/dm_dyxp.jsp?ywxh=" + salesorderid, "Print",
				 ...
				 * $("#selfprivilege").val() - $("#marketprivilege").val() -
				 * $("#vipintegral").val(); $("#cash").val(_usercash); _unionpay =
				 * $("#unionpay").val(); _shoppingcard =
				 * $("#shoppingcard").val(); _subscription =
				 ...
				 * '提示', function(r) { $("#saleman").click();
				 * $("#saleman").focus(); }); return false; } else {
	...
	$("#userconfiger").dialog(
			{
				...
				buttons : {
					"保存" : function() {
						...
						$.ajax({
							url : "longhaul/pos/order/orderSystem.ered?reqCode=setUserConfiger&postType=1&userid=" + opterator
									+ "&autocompletewords=" + _autocompletewords + "&autocompletesecond=" + _autocompletesecond + "&random="
									+ Math.random() + "",
							success : function(data) {
	...
	$("#marketprivilege").keyup(function(event) {
		// value = $(this).val();
		// if (checkNum(this)) {
		// unionpayvalue = Number($("#cash").val()) + Number(_marketprivilege);
		// unionpayvalue = Number(unionpayvalue) - Number(value);
		// $("#cash").val(unionpayvalue);
	...
	$("#vipintegral").keyup(function(event) {
		...
		if (checkNum(this)) {
			unionpayvalue = Number($("#realprice").val()) + Number(_vipintegral);
			unionpayvalue = Number(unionpayvalue) - Number(value);
			$("#realprice").val(unionpayvalue);
			_vipintegral = value

			if (Number(value) > kunnrjf + currentInteval) {
				alert("积分不足");
	...
	$("#marketprivilege").keyup(function(event) {
		value = $(this).val();
		if (checkNum(this)) {
			unionpayvalue = Number($("#realprice").val()) + Number(_selfprivilege);
			unionpayvalue = Number(unionpayvalue) - Number(value);
			$("#realprice").val(unionpayvalue);
			_selfprivilege = value
	...
	$("#marketticketprice").keyup(function(event) {
		value = $(this).val();
		if (checkNum(this)) {
			unionpayvalue = Number($("#realprice").val()) + Number(_marketprivilege);
			unionpayvalue = Number(unionpayvalue) - Number(value);
			$("#realprice").val(unionpayvalue);
			_marketprivilege = value
	...
	function register() {
		if ($("#regname").val() == "") {
			...
			$.ajax({
				url : "longhaul/pos/order/orderSystem.ered?reqCode=validateRegistPhone&postType=1&werks=" + WERKS + "&random=" + Math.random() + "",
				dataType : "json",
				type : 'post',
				data : {
					regPhone : $("#regtel").val()
				...
				success : function(data) {
					if (data != "" && data != null) {
						...
						$.ajax({
							url : "longhaul/pos/order/orderSystem.ered?reqCode=registerUser&postType=1&werks=" + WERKS + "&random=" + Math.random()
									+ "",
							dataType : "json",
							type : 'post',
							data : {
	...
	$("#discount12").change(
			function() {
				...
				if ($("#discount12").get(0).selectedIndex == 2) {
					...
					$("#discount1").attr("readonly", true);
					$.ajax({
						url : "longhaul/pos/order/orderSystem.ered?reqCode=getDiscount&option=user&postType=1&charg=" + $("#charg").val() + "&kunnr="
								+ $("#kunnr").val() + "&matnr=" + $("#matnr").val(),
						dataType : 'json',
						success : function(data) {
							if ($("#discount12").get(0).selectedIndex == 0) {
								...
								if ($("#salepromotionSelect").find("option").length == 0) {
									...
									var discount = 100 + (Number($("#salepromotionSelect").val().substring(
											$("#salepromotionSelect").val().indexOf("|") + 1)) / 10);
	...
	$("#salepromotionSelect").change(function(e) {
		$("#salepromotion").val($(this).val().substring(0, $(this).val().indexOf("->")));
		var discount = 100 + Number($(this).val().substring($(this).val().indexOf("|") + 1)) / 10;
		$("#discount1").val(discount);
		calcPrice();
	});
	...
	function calcPrice() {
		...
		}else if (((ordertype == "ZYS2" || ordertype == "ZJM2") && ($("#swaptype").val() == "ZIN"
		// ||$("#swaptype").val()=="GZIN"
		)) || (ordertype == "ZYR1") || (ordertype == "ZJR1")) {
			// if((ordertype == "ZYS2"||ordertype ==
			// "ZJM2")&&($("#swaptype").val()=="ZIN"||$("#swaptype").val()=="GZIN"))
			...
			var receivable = Number($("#receivable").val());
			var goldweight = Number($("#goldweight").val());
			...
			if ($("#swaptype").val() == 'GZIN' && $("#outGoldTypeSpan").css("display") != 'none') {
				var discount1 = 100;
				var discount2 = 100;
			}
			...
			if ($("#goldvalue").css("display") == "none") {
				total = ($("#ztjtj").val() * (discount1 / 100) * (discount2 / 100)).toFixed(2) * doubleorsingle;
				$("#realprice").val(total.toFixed(2));
				$("#receivable").val(total.toFixed(2));
			} else {
				total = ((goldValue * goldWeight * (discount1 / 100) * (discount2 / 100)) + (Number($("#personcost").val()) * goldWeight) - depreciationPrice)
						* doubleorsingle;
				$("#realprice").val(total.toFixed(2));
				$("#receivable").val(total.toFixed(2));
			}
	...
	function getGiftInfo() {
		...
		$.ajax({
			url : "longhaul/pos/order/orderSystem.ered?reqCode=getpcxx&postType=1&ordertype=" + _ordertype + "&option=auto&werks=" + WERKS
					+ "&random=" + Math.random() + "",
			dataType : "json",
			data : "charg=" + $.trim(_charg) + "&chargtype=" + _chargtype + "&giftReferencePrice=" + $("#giftReferencePrice").val(),
			success : function(data) {
				...
				if (data == "") {
					...
					chargalertstr = _chargtype == "gift" ? "没有找到整单礼品信息!" : chargalertstr
					jAlert(chargalertstr, '提示', function(r) {
						clearcharginfo();
						$("#matnrSelect").hide();
						$("#charg").show();
				...
				$.map(data, function(item) {
					...
					$("#matnrSelect").append($("<option value='" + matnr + "'>" + plmc + '->' + matnr + "</option>"));
				});
			}
		});
	...
	function getPackageInfo() {
		...
		$.ajax({
			url : "longhaul/pos/order/orderSystem.ered?reqCode=getPackageInfo&postType=1&ordertype=" + _ordertype + "&option=auto&werks=" + WERKS
					+ "&random=" + Math.random() + "",
			dataType : "json",
			data : "charg=" + $.trim(_charg) + "&chargtype=" + _chargtype + "&giftReferencePrice=" + $("#giftReferencePrice").val(),
			success : function(data) {
	...
	$("#matnrSelect").change(function(e) {
		...
		if ($(this).get(0).selectedIndex != 0) {
			...
			}else if($("#inputgoldbar").attr("checked") == "checked" || ordertype == "ZYS4" || ordertype == 'ZJM4'){
				getmatnrbyuser();
			} else {
				getchargbyuser();
				$(this).val('');
	...
	function getLgort(lgort) {
		...
		$.ajax({
			url : "longhaul/pos/order/orderSystem.ered?reqCode=getLgort&option=user&postType=1&werks=" + "1000" + "&random=" + Math.random() + "",
			dataType : "json",
			success : function(data) {
				$.map(data, function(item) {
					$("#logrt").append(
							"<option value='" + item.lgort + "'" + (item.lgort == lgort ? "checked" : "") + ">" + item.lgort + item.lgobe
									+ "</option>");
				});
			}
		});
	...
	function submit() {
		if (opmode == "view" && useroption == "print") {
			_salesorderid = $("#salesorderid").val();
			window.showModalDialog(basepath + "longhaul/pos/order/dm_dyxp.jsp?ywxh=" + _salesorderid + "&dwbh=" + WERKS, 'printWindow',
					'dialogWidth=650px;dialogHeight=700px');
			$('input').attr('disabled', true);
			$('select').attr('disabled', true);
			return;
				...
				if (retdata.sapsalesorderid == null || retdata.sapsalesorderid == "") { // 如果SAP订单号为空
					if (retdata.salesorderid != null || retdata.salesorderid != "") { // 如果SAP没有保存成功,但是AIG保存成功
						...
						if (useroption == "print") {
							...
							// 'top=10,width=650,height=800,scrollbars=yes,
							// resizable=yes');
							window.showModalDialog(basepath + "longhaul/pos/order/dm_dyxp.jsp?ywxh=" + retdata.salesorderid + "&dwbh=" + WERKS,
									'printWindow', 'dialogWidth=650px;dialogHeight=700px');
							$('input').attr('disabled', true);
							$('select').attr('disabled', true);
						} else {
					...
					if (useroption == "print") {
						...
						messagestr = messagestr + "<br><font  class='printinfo'>是否打印?</font>";
						// jAlert(_retinfo, '提示', function(r) {
						jConfirm(messagestr, '提示', function(r) {
							if (r == true) {
								...
								// cleartable();
								// clearhead();
								window.showModalDialog(basepath + "/longhaul/pos/order/dm_dyxp.jsp?ywxh=" + retdata.salesorderid, 'Print',
	...
	function initialWindow(orderType) {
		if (orderType == 'ZJM3' || orderType == 'ZJR2' || orderType == 'ZYS3' || orderType == 'ZYR2' || orderType == 'ZYS8' || orderType == 'ZJM8'
				|| orderType == 'ZYR3' || orderType == 'ZJR3') {
			$("#groupSealSpan").hide();
		...
		} else if (orderType == 'ZJM6' || orderType == 'ZYS6' || orderType == 'ZJM4' || orderType == 'ZYS4' ) {
			$("#groupSealSpan").hide();
			$("#orderreasonSpan").hide();
			$("#discount12Span").hide();
			$("#discount34Span").hide();
		...
		if (ordertype == "ZYS2" || ordertype == "ZJM2" || ordertype == "ZYR1" || ordertype == "ZJR1") {
			$("#saleorderidspan").show();
		...
		$.ajax({
			...
			success : function(retdata) {
				$.map(retdata, function(item) {
					...
					$("#statementtotal").val((Number(totalMoney) * 0.3).toFixed(0));
					$("#cash").val((Number(totalMoney) * 0.3).toFixed(0));
				});
	...
	function creatChoiceOrderRow(data) {
		...
		$.each(data, function(key, val) {
			...
			_ordertype = $("#ordertype").val(); // 订单类型字段
			style = "tdtoplevel";
			ordertItemype = "<div id=ordertItemype" + key + " style='display:none'>ZL2N</div>";
			storagelocation = val.storagelocation == null ? "" : val.storagelocation;
			_logrt = "<div id=logrt" + key + ">" + storagelocation + "</div>";
			delimage = "<img alt='删除' src='resource/image/ext/delete.png' id='delrow'  align='middle' style='cursor:hand;'>";
			row = "<div id=" + key + ">" + "" + "</div>";
			...
			toplevelstr = "<div id=leveladdgifts" + val.salesorderitem + "  style='display:none'>" + lowerlevel + "</div>";
			posnrnumber = "<div  id=posnrnumber" + key + "> " + val.choiceorderitem + " </div>" + "<div id=upnumber" + _chargtype + key
					+ "  style='display:none'>" + val.choiceorderitem + "</div>" + toplevelstr + "";
			upumber = "<div id=upnumberlevel" + key + ">" + val.upchoiceorderitem + "</div>";
			batchnumber = val.batchnumber == null ? "" : val.batchnumber;
			_charg = "<div id=charg" + key + ">" + batchnumber + "&nbsp;</div>"; // 批次
			var materialnumber = val.materialnumber == null || val.materialnumber == "null" ? "" : val.materialnumber;
			_matnr = "<div id=matnr" + key + ">" + materialnumber + "&nbsp; </div>"; // 物料
			...
			_realprice = "<div id=realprice" + key + ">" + Number(Number(totalMoney) * 0.3).toFixed(0) + "</div>"; // 实销价格
			...
			imaghref = "<a href=" + imgpath + " class='tooltip'>";
			_pcimage = "<div id=pcimage" + key + ">" + imaghref + "<img id=pcimagesrc   src=sappic/" + imgpath + " height='40' width='38' alt='"
					+ _productpictureurl + "' onerror='this.src=\"longhaul\/pos\/order\/images\/zjzb.gif\"'/></a></div>";
			...
			$("#totalztjtj").text((Number($("#totalztjtj").text()) + Number(totalMoney)).toFixed(0)); // 
	...
	function getIntegralMatnrInfo() {
		var matnr = $("#matnrSelect").val().substring(0, $("#matnrSelect").val().indexOf("-"));
		var integral = $("#matnrSelect").val().substring($("#matnrSelect").val().indexOf("-") + 1);
		$.ajax({
	...
	function creatFrontMoneyItemRow(data) {
		...
		$.each(data, function(key, val) {
			...
			_ordertype = $("#ordertype").val(); // 订单类型字段
			style = "tdtoplevel";
			ordertItemype = "<div id=ordertItemype" + key + " style='display:none'>ZG2N</div>";
			storagelocation = val.storagelocation == null ? "" : val.storagelocation;
			_logrt = "<div id=logrt" + key + ">" + storagelocation + "</div>";
			delimage = "<img alt='删除' src='resource/image/ext/delete.png' id='delrow'  align='middle' style='cursor:hand;'>";
			row = "<div id=" + key + ">" + "" + "</div>";
			...
			_realprice = "<div id=realprice" + key + " style='display : none;'>" + Number(val.netprice).toFixed(0) + "</div><div>-"
			...
			success : function(retdata) {
				...
				if (retdata.length == 0) {// retdata.length==0){
					...
					$.map(retdata, function(item) {
						...
						$("#salepromotionSelect").append(
								"<option value='" + item.aktnr + '->' + item.aktkt + '|' + item.kbetr + "' kbetr='" + item.kbetr + "'>" + item.aktnr
										+ '->' + item.aktkt + "</option>");
					});
				}
			}
	...
	function orderTypeCheck() { // 判断订单类型
		if (ordertype == 'ZJM3' || ordertype == 'ZJR2' || ordertype == 'ZYS3' || ordertype == 'ZYR2' || ordertype == 'ZYR3' || ordertype == 'ZYS8'
				|| ordertype == 'ZJM8' || ordertype == 'ZJR3' || ordertype == 'ZOR6' || ordertype == 'ZYS6' || ordertype == 'ZJM6'|| ordertype == 'ZYS4' || ordertype == 'ZJM4')
			return false;
		else
			return true;
	}
		...
		if (event.keyCode == 13) {
	...
	}).autocomplete(
			{
				source : function(request, response) {
					...
					$.ajax({
						url : "longhaul/pos/order/orderSystem.ered?reqCode=getMatnrInfo&postType=1&ordertype=" + _ordertype + "&option=auto&werks="
								+ WERKS + "&random=" + Math.random() + "",
						dataType : "json",
						...
						success : function(data) {
							...
							response($.map(data, function(item) {
								var matnr = item.matnr == null ? "" : item.matnr; // 物料号
								var maktx = item.maktx == null ? "" : item.maktx; // 产品名称
								return {
									label : matnr + "->" + maktx
								};
							}));
	...
	function getmatnrbyuser() {
		...
		if ($("#outGoldTypeSpan").css("display") != 'none' && $("#swaptype").val() == 'GZIN') {
		...
		$.ajax({
			...
			success : function(data) {
				...
				$.map(data, function(item) {
					...
					if ($("#swaptype").val() == 'GZIN' && zjlbm != null && zjlbm != '') {
						getgoldprices(zjlbm);
					}
	...
	function creatGroupGoodsRow(data) {
		...
		for ( var i = 0; i < data.length; i++) {
			if (Number(data[i].labst) <= 0) {
				jAlert("组合中批次<b style='color:red;'>" + data[i].cpbm + "-" + data[i].plmc + "</b>库存不足，不能组合销售！", "提示", function(e) {
					$("#charg").select();
				});
				return;
			}
		...
		for ( var i = 0; i < chargDivs.length; i++) {
			for ( var i = 0; i < data.length; i++) {
				if ($.trim($(chargDivs[i]).text()) == $.trim(data[i].cpbm)) {
					jAlert("组合中已经有批次<b style='color:red;'>" + data[i].cpbm + "-" + data[i].plmc + "</b>录入到销单中，请删除后重新录入！", "提示", function(e) {
						$("#charg").select();
					});
					return;
				}
		...
		var _marketprivilege = (Number($("#marketprivilege").val()) / 2).toFixed(0);
		var _selfprivilege = (Number($("#selfprivilege").val()) / 2).toFixed(0);
		var _marketticketprice = (Number($("#marketticketprice").val()) / 2).toFixed(0);
		var _selfticketprice = (Number($("#selfticketprice").val()) / 2).toFixed(0);
		...
		$.each(data, function(key, val) {
			...
			var charg = "<div id=charg" + rownum + ">" + val.cpbm + "&nbsp;</div>"; // 批次
			var matnr = "<div id=matnr" + rownum + ">" + val.bkbh + "&nbsp; </div>"; // 物料

			var zhlhx = "<div id=zhlhx" + rownum + " class='divmatnrdesc'>" + val.plmc + "&nbsp; </div>"; // 商品ID
			var uniteprice = "<div id=uniteprice" + rownum + ">" + Number($("#ztjtj").val()).toFixed(0) + "&nbsp;</div>"; // 单价
			var number = "<div id=number" + rownum + ">" + _number + "</div>"; // 数量
			var personcost = "<div id=personcost" + rownum + ">" + Number($("#personcost").val()).toFixed(0) + "</div>"; // 人工费用
			var goldweight = "<div id=goldweight" + rownum + ">" + val.jlzl + "</div>"; // 金重
			var goldvalue = "<div id=goldvalue" + rownum + " style='display:none'>" + $("#goldvalue").val() + "</div>"; // 金价

			var goldrealprice = "<div id=goldrealprice" + rownum + ">"
					+ (Number($("#personcost").val()) + Number($("#goldweight").val()) * Number($("#goldvalue").val())).toFixed(0) + "&nbsp;</div>"; // 金价初始总价

			var receivable = "<div id=receivable" + rownum + ">" + Number(_total).toFixed(0) + "</div>"; // 合计
			var ztjtj = "<div id=ztjtj" + rownum + ">" + val.bqj + "</div>"; // 标签价格
			var realTagPrice = "<div id=realTagPrice" + rownum + ">" + val.bqj + "</div>"; // 标签价格
			_realprice = Number(
					Number(val.bqj) * (_discount1 != "N/A" ? (Number(_discount1) / 100) : 1) * (_discount2 != "N/A" ? (Number(_discount2) / 100) : 1)
							* (_discount3 != "N/A" ? (Number(_discount3) / 100) : 1) * (_discount4 != "N/A" ? (Number(_discount4) / 100) : 1)
							* (_discount5 != "N/A" ? (Number(_discount5) / 100) : 1)).toFixed(0);
			_realprice = _realprice - _vipintegral;
			// alert(_realprice);
			var realprice = "<div id=realprice" + rownum + " >" + (_realprice) + "</div>"; // 实销价格

			var salepromotion = "<div id=salepromotion" + rownum + ">"
					+ (($("#discount12").get(0).selectedIndex == 1) ? $("#salepromotion").val() : "") + "</div>"; // 促销代码

			var discount1 = "<div id=discount1" + rownum + ">" + _discount1 + "</div>"; // 折扣1
			var discount2 = "<div id=discount2" + rownum + ">" + _discount2 + "</div>"; // 折扣2
			var discount3 = "<div id=discount3" + rownum + ">" + _discount3 + "</div>"; // 折扣3
			var discount4 = "<div id=discount4" + rownum + ">" + _discount4 + "</div>"; // 折扣4
			var discount5 = "<div id=discount5" + rownum + ">" + _discount5 + "</div>"; // 折扣5

			var marketprivilege = "<div id=marketprivilege" + rownum + ">" + marketprivilege + "</div>";
			var selfprivilege = "<div id=selfprivilege" + rownum + ">" + selfprivilege + "</div>";
			var marketticketprice = "<div id=marketticketprice" + rownum + ">" + marketticketprice + "</div>";
			var selfticketprice = "<div id=selfticketprice" + rownum + ">" + selfticketprice + "</div>";
			// var vipintegralNum = Number($("#vipintegral").val());
			var vipintegral = "<div id=vipintegral" + rownum + ">" + _vipintegral + "</div>";
			var currentIntegralNum = parseInt(Number($("#currentIntegral").val()) == 0 ? 0
			...
			$("#toalpersoncost").text((Number($("#toalpersoncost").text()) + Number($("#personcost").val())).toFixed(0)); // 人工费用
			$("#totalgoldweight").text((Number(Number($("#totalgoldweight").text()) + _goldweight))); // 总金重
			// if (chargtype == "charg" || _ordertype == 'ZRE2') {
			$("#total").text((Number($("#total").text()) + _total).toFixed(0)); // 
			$("#totalztjtj").text((Number($("#totalztjtj").text()) + Number(_ztjtj)).toFixed(0)); // 
			$("#totalrealprice").text(
					(Number($("#totalrealprice").text()) + (Number(val.bqj) * (_discount1 != "N/A" ? (Number(_discount1) / 100) : 1)
							* (_discount2 != "N/A" ? (Number(_discount2) / 100) : 1) * (_discount3 != "N/A" ? (Number(_discount3) / 100) : 1)
							* (_discount4 != "N/A" ? (Number(_discount4) / 100) : 1) * (_discount5 != "N/A" ? (Number(_discount5) / 100) : 1)))
							.toFixed(0)); // 实际销售合计
			// }
			$("#goldvalue").removeClass("inputattention");
			$("#personcost").removeClass("inputattention");
			...
			row = row + "<td  id='goldtr" + rownum + "'  class=" + style + "  align='right' style=" + goodtdshow + ">" + goldweight + goldvalue
					+ "</td>";
			row = row + "<td style='display:none' class=" + style + " align='right'>" + receivable + "</td>";
			...
			if (ordertype != "ZJM6" && ordertype != "ZYS6" && ordertype != "ZOR6" && ordertype != "ZJM4" && ordertype != "ZYS4") {
				row = row + "<td class=" + style + ">" + discount1 + "</td>";
				row = row + "<td class=" + style + ">" + discount2 + "</td>";
				row = row + "<td class=" + style + ">" + discount5 + "</td>";
				row = row + "<td class=" + style + ">" + discount3 + "</td>";
...
//			}
		...
		$.ajax({
			...
			success : function(data) {
				for ( var i = 0; i < data.length; i++) {
					...
					var text = item.maktx;
					var option = $("<option value='" + value + "'>" + text + '-' + value + "</option>")
	...
	function getcurrentInteval() {
		...
		$("#tablecontent tr").each(function() { // 得到每一个tr
			earchobject = $(this);
			var currentIntevial = earchobject.find("div[id^='currentIntegral']").text();
			var vipintegral = earchobject.find("div[id^='vipintegral']").text();
			totalCurrentIntetial += Number(currentIntevial);
			totalVipintegral += Number(vipintegral);
		});

		$("#currentIntevalSpan").text((totalCurrentIntetial - totalVipintegral).toFixed(0));
	...
	function getPersonCost(){
		if ((ordertype == 'ZJM2' || ordertype == 'ZYS2') && $("#swaptype").val() == 'ZIN') {
			...
			switch (mykondm) {
			case '09':
				if (Number($("#goldweight").val()) < 1) {
					$("#personcost").val(20);
					// $("#personcostshow").val(20);
					//getgoldprices(sjczbm);
				} else if (Number($("#goldweight").val()) >= 1) {
					$("#personcost").val(10);
					// $("#personcostshow").val(10);
					//getgoldprices(sjczbm);
				}
		...
		$.ajax({
			...
			success : function(data) {
				...
				$.map(data, function(item) {
					...
					var option = $("<option value='" + matnr+ "->" + labst + "'>" + plmc + '->' + matnr + "</option>");
