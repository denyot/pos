		...
		$.ajax({
			...
			success : function(data) {
				$.map(data, function(item) {
					...
					var option = $("<option value='" + item.atwrt + "'" + (flag ? "selected" : "") + " >" + item.atwrt +' | '+ item.atwtb + "</option>");
...
//				});
	...
	function getMytechnics() {
		...
		$.ajax({
			...
			success : function(data) {
				$.map(data, function(item) {
					var option = $("<option value='" + item.atwrt + "' >" + item.atwrt+' | '+item.atwtb + "</option>");
...
//				});
	...
	$("#toneType").change(function(){
		needGetMatnr += 1;
		//getNewPrice();
		if($("#toneType").get(0).selectedIndex != 0){
...
// 获取石头颜色
	...
	// 获取玉石颜色
	function getToneColor2(type, value) {
		$("#toneColor2").empty();
		$("#toneColor2").append($("<option  value=''>请选择...</option>"));
		var types = "";
		switch (type) {
		case "X1":
			types = "00,99,98,97,96,95,94,93,92,91,90,89,88,87,86,85,84,Y,BR,GR,G,R,R1,BL";
			break;
		case "X2":
			types = "R,R1,R2,B,B1,B2,G,Y,O,P,W";
			break;
		case "X3":
			types = "B,B1,B2,B3,B4,B5,B6,BL,BR,BR1,D,G,G1,G2,G3,G4,G5,G6,GO,GR,P,P1,PU,R,R1,R2,R3,R4,R5,R6,W,W1,Y,Y1";
			break;
		case "X4":
			types = "W,Y,P,O,B,BR,BL";
			break;
	...
	function getMyCertificate() {
		...
		$.ajax({
			...
			success : function(data) {
				$.map(data, function(item) {
					var option = $("<option value='" + item.zzslx + "'>" + item.zzslx+' | '+item.tzslx + "</option>");
...
//				});
	...
	getWerksInfo();
	function getWerksInfo() {
		$("#werksInfo").empty();
		$.ajax({
			url : 'longhaul/pos/choiceorder/choiceOrderSystem.ered?reqCode=getWerksInfo&option=user&postType=1',
			dataType : "json",
			data : "vkorg=1000",
			success : function(data) {
				$.map(data, function(item) {
					var option = $("<option value='" + item.werks + "'>" + item.werks + " | " + item.name1 + "</option>");
					$("#werksInfo").append(option);
				});
//				var noneselectedtext = "请选择...";
//				$("#certificate").multiSelect({
//					selectAll : false,
			...
			$.ajax({
				...
				success : function(data) {
					if (data == "") {
						...
						jAlert(chargalertstr,"提示",function(e){clearcharginfo();});
						response(null);
						return;
					}
		...
		$.ajax({
			...
			success : function(data) {
				...
				$.map(data, function(item) {
					...
					var zcpxl = item.zcpxl;
					var zcpfg = item.zcpfg;
...
//		$("#matnrinput").css("display","none");
		...
		var zcpxl = $.trim(charginfo.zcpxl);
		var zcpfg = $.trim(charginfo.zcpfg);
...
//		}
		...
		if(charginfo.bismt == "" || charginfo.bismt == null){
			...
			if (orderType == 2 || orderType == 3)
				...
				getBismtInfo(charginfo.bismt,slxz,slgg,zgy,sjczbm,slbm,zcpxl,zcpfg);
	...
	function getCurrentPrice(kondm, slbm, gemweight, slxz, slys, zsjd, zgy, extwg) {
		if (orderType == 1) {
			...
			$.ajax({
				...
				dataType : "json",
				type : "POST",
				data : "kondm=" + kondm + "&charg=" + charg + "&toneType=" + slbm + "&gemweight=" + gemweight + "&toneColor=" + slys + "&toneNeatness=" + zsjd + "&technics=" + zgy + "&extwg=" + extwg,
				success : function(data) {
	...
	function getNewPrice() {
		...
		if(needGetMatnr != 0){
				jAlert("你有修改选款参数值，需要重新获取物料号！","提示",function(e){
					$("#getMatnrInfo").focus();
				});
				return;
			}
		...
		var ifNeedLessTone = $("#ifNeedLessTone").attr("checked");
			if(ifNeedLessTone == "checked"){
				if(lessToneType == ''){
					jAlert("请选择副石石料！","提示",function(e){
						$("#lessToneType").focus();
					});
					return;
				}
		...
		if (orderType == 1) {
			...
			if (charg != "" || matnr != "") {
				$.ajax({
					...
					type : "POST",
					data : 
						"&toneType=" + $("#toneType").val() + "&gemweight=" + $("#gemweight").val() + "&matkl=" + $("#matkl").val()+"&matnr=" + matnr+"&charg=" + charg+"&goldType=" + goldType+"&lessToneType=" +
						lessToneType+"&toneColor=" + toneColor + "&toneNeatness=" + toneNeatness + "&toneCount="+toneCount,
//					"&toneType=" + $("#toneType").val() + "&gemweight=" + $("#gemweight").val() + "&toneColor=" + $("#toneColor").val() + "&toneNeatness=" + $("#toneNeatness").val() + "&technics="
//							+ $("#technics").val() + "&extwg=" + $("#extwg").val() + "&matkl=" + $("#matkl").val()+"&matnr=" + matnr+"&charg=" + charg,
					success : function(data) {
						dialog.close();
...
//						$("#retailPrice").val(newPrice > 0 ? newPrice.toFixed(0) : 0);
//						if(newPrice <= 0) $("#retailPrice").attr("readOnly",false);
...
//		$("#mainToneStyle").append("<option value=''>请选择...</option>");
	...
	$("#remark").keydown(function(event) {
		if (event.keyCode == 13) {
			...
			if (orderType == 1) {
				if ($("#goldType").get(0).selectedIndex == 0) {
					alert("请选择金料类型!");
						$("#goldType").focus();
					return;
				}
				...
				if ($("#toneType").get(0).selectedIndex == 0) {
					alert("请选择石料类型!");
						$("#toneType").focus();
					return;
				} else if ($("#toneColor").get(0).selectedIndex == 0 && $("#toneColor").find("option").length>1 && !isHavePrice && $("#type").val() != 'X4'
					 && $("#type").val() != 'X3' && $("#type").val() != 'X2' && !isHavePrice) {
					alert("请选择石料颜色!");
					$("#toneColor").focus();
					return;
				} else if ($("#toneNeatness").get(0).selectedIndex == 0 && $("#type").val() == "X1" && !isHavePrice) {
					alert("请选择石料净度!");
					$("#toneNeatness").focus();
					return;
				} else if ($("#goldType").get(0).selectedIndex == 0) {
					alert("请选择金料类型!");
					$("#goldType").focus();
					return;
				}
			} else if (orderType == 3) {
				if ($("#bowlderType").get(0).selectedIndex == 0) {
					alert("请选择石料类型!");
						$("#bowlderType").focus();
					return;
//				} else if ($("#toneshape").get(0).selectedIndex == 0) {
//					alert("请选择石料形状!");
//						$("#toneshape").focus();
//					return;
			...
			if(orderType == 1 && $("#type").val() != "J3"){
				if($("#goldweightFrom").val() == ''){
					alert("您没有输入金重的起始值！");
						$("#goldweightFrom").focus();
					return;
				}else if( $("#goldweightTo").val() == ''){
					alert("您没有输入金重的结束值！");
						$("#goldweightTo").focus();
					return;
				}
...
//			var choiceType = $("#type").val();
//			if(choiceType == 'J1' || choiceType == 'J2' || choiceType == 'J3' || choiceType == 'Y1'|| choiceType == 'X2'|| choiceType == 'X3'){
//				//alert($("#remark").val());
//				if($("#remark").val() == ''){
...
//		}
		...
		var delimage = "<img alt='删除' src='resource/image/ext/delete.png' id='delrow'  align='middle' style='cursor:hand;'><img alt='复制' src='resource/image/ext/add.png' id='copyrow'  align='middle' style='cursor:hand;'>";
		...
		if (orderType == 3)
			var toneType = "<div id=toneType" + rownum + " style='display:none'>" + ($("#bowlderType").get(0).selectedIndex != 0 ? $("#bowlderType").val()+"|-|" + $("#bowlderType").find("option:selected").text() : "") + "</div><div id='toneTypeShow"+rownum+"'>"
					+ ($("#bowlderType").get(0).selectedIndex != 0 ? $("#bowlderType").find("option:selected").text() : "&nbsp;") + "</div>"; // 石料
		else
			var toneType = "<div id=toneType" + rownum + " style='display:none'>" + ($("#toneType").get(0).selectedIndex != 0 ? $("#toneType").val()+ "|-|"+ $("#toneType").find("option:selected").text() : "") + "</div><div id='toneTypeShow"+rownum+"'>"
		...
		// 总重
		var goodTotalWeight = "<div id=goodTotalWeight" + rownum + ">" + Number($("#goodTotalWeight").val()) + "</div>";

		// 石料颜色
		var toneColor = "<div id=toneColor" + rownum + " style='display:none'>"
				+ (orderType == 2 ? ($("#toneColor").get(0).selectedIndex != 0 ? ($("#toneColor").val() + "|-|"+$("#toneColor").find("option:selected").text()) : "") : ($("#toneColor2").get(0).selectedIndex != 0 ? ($("#toneColor2").val() + "|-|"+$("#toneColor2").find("option:selected").text()) : "")) + "</div><div id='toneColorShow"+rownum+"' >"
				+ (orderType == 2 ? ($("#toneColor").get(0).selectedIndex != 0 ? $("#toneColor").find("option:selected").text() : "&nbsp;") : ($("#toneColor2").get(0).selectedIndex != 0 ? $("#toneColor2").find("option:selected").text() : " "))
				+ "</div>";

		// 石料净度
		var toneNeatness = "<div id=toneNeatness" + rownum + " style='display:none'>" + ($("#toneNeatness").get(0).selectedIndex != 0 ? ($("#toneNeatness").val() + "|-|" + $("#toneNeatness").find("option:selected").text()) : " ") + "</div><div id='toneNeatnessShow"+rownum +"'>"
				+ ($("#toneNeatness").get(0).selectedIndex != 0 ? $("#toneNeatness").find("option:selected").text() : "&nbsp;") + "</div>";

		// 石料形状$("#toneshapeth").css("display", "none");
		var toneshape = "<div id=toneshape" + rownum + " style='display:none'>" + ($("#toneshape").get(0).selectedIndex != 0 ? ($("#toneshape").val()+ "|-|" + $("#toneshape").find("option:selected").text()) : "") + "</div><div id='toneNeatnessShow"+rownum +"'>"
				+ ($("#toneshape").get(0).selectedIndex != 0 ? $("#toneshape").find("option:selected").text() : "&nbsp;") + "</div>";
				
		// 风格
		var styleDiv = "<div id=style" + rownum + " style='display:none'>" + ($("#style").get(0).selectedIndex != 0 ? ($("#style").val()+ "|-|" + $("#style").find("option:selected").text()) : "") + "</div><div id='styleShow"+rownum +"'>"
				+ ($("#style").get(0).selectedIndex != 0 ? $("#style").find("option:selected").text() : "&nbsp;") + "</div>";
		
		// 系列
		var seriesDiv = "<div id=series" + rownum + " style='display:none'>" + ($("#series").get(0).selectedIndex != 0 ? ($("#series").val()+ "|-|" + $("#series").find("option:selected").text()) : "") + "</div><div id='seriesShow"+rownum +"'>"
				+ ($("#series").get(0).selectedIndex != 0 ? $("#series").find("option:selected").text() : "&nbsp;") + "</div>";

		// 金料
		var goldType = "<div id=goldType" + rownum + " style='display:none'>" + ($("#goldType").get(0).selectedIndex != 0 ? ($("#goldType").val()+ "|-|" +$("#goldType").find("option:selected").text())  : "") + "</div><div id='goldtypeShow"+rownum +"'>"
				+ ($("#goldType").get(0).selectedIndex != 0 ? $("#goldType").find("option:selected").text() : "&nbsp;") + "</div>";

		// 工艺
		var	technics = $('#technics').text().split(',');
		var technicsValues = $("#technics").selectedValuesString().split(',');
		var technicsText = $('#technics').find("option:selected").text()
		for(var i = 0;i<technics.length;i++)
			technicsText+= technics[i]+'<br/>';
		technicsText = $('#technics').find("option:selected").text();
		var technics = "<div id=technics" + rownum + " style='display:none'>" + ($("#technics").val() != "" ? ($("#technics").val()+"|-|" + technicsText) : "") + "</div><div id='technicsShow"+rownum +"'>"
				+ ($('#technics').get(0).selectedIndex != 0 ? technicsText : "&nbsp;") + "</div>";
		...
		var selectWerks = "<div id=selectWerks" + rownum + " style='display : none;'></div><div id=selectWerksShow" + rownum + " style='height:50px; overflow:hidden'>" + "&nbsp;" + "</div>"; // 门店
		...
		// 副石
		var lessToneTypeDiv = "<div id=lessToneType" + rownum + " style='display:none'>" + (($("#ifNeedLessTone").attr("checked") == "checked") ? ($("#lessToneType").get(0).selectedIndex != 0 ? ($("#lessToneType").val() +"|-|" + $("#lessToneType").find("option:selected").text()) : "") : '')
				+ "</div><div id='lessToneTypeShow"+rownum +"'>" + (($("#ifNeedLessTone").attr("checked") == "checked") ? ($("#lessToneType").get(0).selectedIndex != 0 ? $("#lessToneType").find("option:selected").text() : "&nbsp;") : '&nbsp;') + "</div>";

		// 证书
		var	certificate = $('#certificate').text().split(',');
		var certificateValues = $("#certificate").selectedValuesString().split(',');
		var certificateText = '';
		for(var i = 0;i<certificate.length;i++)
			certificateText+= certificate[i]+'<br/>';
				
				
		var certificateDiv = "<div id=certificate" + rownum + " style='display:none'>" + ($("#certificate").selectedValuesString() != "" ? ($("#certificate").selectedValuesString() + "|-|" + certificateText) : "") + "</div><div id='certificateShow"+rownum +"' style='width:80px;height:50px;overflow:hidden;'>"
				+ ($('#certificate').text() != "请选择..." ? certificateText : "&nbsp;") + "</div>";

		// 火彩颜色
		var toneFireColor = "<div id=toneFireColor" + rownum + " style='display:none'>" + ($("#toneFireColor").get(0).selectedIndex != 0 ? ($("#toneFireColor").val() + "|-|" +$("#toneFireColor").find("option:selected").text()) : "") + "</div><div id='toneFireColorShow"+rownum +"'>"
				+ ($("#toneFireColor").get(0).selectedIndex != 0 ? $("#toneFireColor").find("option:selected").text() : " ") + "</div>";
		// 主石规格
		var mainToneStyle = "<div id=mainToneStyle" + rownum + " style='display:none'>" + ($("#mainToneStyle").get(0).selectedIndex != 0 ? ($("#mainToneStyle").val()+"|-|" + $("#mainToneStyle").find("option:selected").text()) : "") + "</div><div id='mainToneStyleShow"+rownum +"'>"
				+ ($("#mainToneStyle").get(0).selectedIndex != 0 ? $("#mainToneStyle").find("option:selected").text() : "&nbsp;") + "</div>";

		var goldweight = Number($("#goldweight").val()).toFixed(3);
		...
		var isHavePriceDiv = "<div id=isHavePrice" + rownum + ">" + (isHavePrice ? 1 : 0) + "</div>"; // 旧市场价
...
//		alert(totalsubtotals[1]);
		$("#totalsubtotal").text((Number(Number(totalsubtotals[0]) + Number(retailPrices[0])*Number($("#number").val())))+'-'+(Number(Number(totalsubtotals[1]) + Number(retailPrices[1])*Number($("#number").val())))); // 总价格
		...
		row = row + "<td class=" + style + " align='center'>" + styleDiv + "</td>";
		row = row + "<td class=" + style + " align='center'>" + seriesDiv + "</td>";
...
//		}
		...
		if (orderType == 1 || orderType == 2 || $.trim(ifinlay) == "X")
			row = row + "<td class=" + style + " align='right'>" + goldType + "&nbsp;</td>";
		else if (orderType == 3 && $.trim(ifinlay) == "") {
			row = row + "<td class=" + style + " align='right'>" + '' + "&nbsp;</td>";
		}
		...
		//if (orderType != 1 || $("#type").val() == "J1"||$("#type").val() == "J2"||$("#type").val() == "J4"){
...
//		if (orderType == 2)
//			row = row + "<td class=" + style + " align='right'>" + goodTotalWeight + "</td>"; // 总重

		row = row + "<td class=" + style + " align='right'>" + remark + "</td>"; // 备注
		
		row = row + "<td class=" + style + " width='auto'>" + selectWerks + "</td>"; // 门店
		...
		row = row + "<td class=" + style + "  style='display:none'>" + isHavePriceDiv + "</td>";
	...
	$('#copyrow').live('click', function() {
		...
		var _tempcharg = "", _tempmatnr = "";
		_tempcharg = thistr.find("div[id^=charg]").html();
		_tempmatnr = thistr.find("div[id^=matnr]").html();
		...
		var zhlhx = "<div id=zhlhx" + rownum + " class='divmatnrdesc'>" + thistr.find("div[id^=zhlhx]").html() + "&nbsp; </div>"; // 商品ID
		var number = "<div id=number" + rownum + ">" + thistr.find("div[id^=number]").html() + "</div>"; // 数量
		
		var ifinlay = $(thistr.find("div[id^=ifinlay]")[0]).html() ==  'X' ? 'X' : ' ';
		
		var ifinlayDiv = "<div id=ifinlay" + rownum + " style='display:none'>" + ifinlay + "</div><div id='ifinlayShow"+rownum+"'>" + (ifinlay == "X" ? "是" : "否") + "</div>"; // 是否需要镶嵌

		var toneType = "<div id=toneType" + rownum + " style='display:none'>" + $(thistr.find("div[id^=toneType]")[0]).html() + "</div><div id='toneTypeShow"+rownum+"'>"
					+ $(thistr.find("div[id^=toneType]")[1]).html() + "</div>"; // 石料

		var toneWeight = Number($(thistr.find("div[id^=toneWeight]")[0]).html());
		var toneWeightDiv = "<div id=toneWeight" + rownum + ">" + toneWeight + "</div>"; // 主石重

		// 总重
		var goodTotalWeight = "<div id=goodTotalWeight" + rownum + ">" + $(thistr.find("div[id^=goodTotalWeight]")[0]).html() + "</div>";

		// 石料颜色
		var toneColor = "<div id=toneColor" + rownum + " style='display:none'>"
				+ ($(thistr.find("div[id^=toneColor]")[0]).html()) + "</div><div id='toneColorShow"+rownum+"' >"
				+ ($(thistr.find("div[id^=toneColor]")[1]).html())
				+ "</div>";
	
		// 石料净度
		var toneNeatness = "<div id=toneNeatness" + rownum + " style='display:none'>" + ($(thistr.find("div[id^=toneNeatness]")[0]).html()) + "</div><div id='toneNeatnessShow"+rownum +"'>"
				+ ($(thistr.find("div[id^=toneNeatness]")[1]).html()) + "</div>";

		// 石料形状$("#toneshapeth").css("display", "none");
		var toneshape = "<div id=toneshape" + rownum + " style='display:none'>" + ($(thistr.find("div[id^=toneshape]")[0]).html()) + "</div><div id='toneNeatnessShow"+rownum +"'>"
				+ ($(thistr.find("div[id^=toneshape]")[1]).html()) + "</div>";
				
				
	
//		// 风格
//		var toneshape = "<div id=style" + rownum + " style='display:none'>" + ($(thistr.find("div[id^=style]")[0]).html()) + "</div><div id='styleShow"+rownum +"'>"
//				+ ($(thistr.find("div[id^=style]")[1]).html()) + "</div>";
		
		// 形状
		var toneshape = "<div id=toneshape" + rownum + " style='display:none'>" + ($(thistr.find("div[id^=toneshape]")[0]).html()) + "</div><div id='toneshape"+rownum +"'>"
				+ ($(thistr.find("div[id^=toneshape]")[1]).html()) + "</div>";

						

		// 金料
		var goldType = "<div id=goldType" + rownum + " style='display:none'>" + ($(thistr.find("div[id^=goldType]")[0]).html()) + "</div><div id='goldtypeShow"+rownum +"'>"
				+ ($(thistr.find("div[id^=goldtypeShow]")[0]).html()) + "</div>";

		// 工艺
		var technics = "<div id=technics" + rownum + " style='display:none'>" + ($(thistr.find("div[id^=technics]")[0]).html()) + "</div><div id='technicsShow"+rownum +"'>"
				+ ($(thistr.find("div[id^=technics]")[1]).html()) + "</div>";

		var goodsize = "<div id=goodsize" + rownum + ">" + $(thistr.find("div[id^=goodsize]")[0]).html() + "</div>"; // 货品尺寸

		var retailPrice = "<div id=retailPrice" + rownum + ">" + $(thistr.find("div[id^=retailPrice]")[0]).html() + "</div>"; // 市场零售价参考

		var remark = "<div id=remark" + rownum + ">" + $(thistr.find("div[id^=remark]")[0]).html()  + "&nbsp;</div>"; // 备注
		
		var selectWerks = "<div id=selectWerks" + rownum + " style='display : none;'></div><div id=selectWerksShow" + rownum + ">" + "&nbsp;" + "</div>"; // 门店
		
		
		
		var littleTotal =  Number() * Number($(thistr.find("div[id^=retailPrice]")[0]).html());// 小计金额
		var littleTotalDiv = "<div id=littleTotal" + rownum + ">" + $(thistr.find("div[id^=littleTotal]")[0]).html() + "</div>"; // 小计金额DIV

		var ifNeedLessTone = $(thistr.find("div[id^=ifNeedLessTone]")[0]).html() ;
		var ifNeedLessToneDiv = "<div id=ifNeedLessTone" + rownum + " style='display:none'>" + ifNeedLessTone + "</div><div id='ifNeedLessToneShow"+rownum+"'>" + (ifNeedLessTone == "X" ? "是" : "否") + "</div>"; // 是否需要副石Div

		// 副石
		var lessToneTypeDiv = "<div id=lessToneType" + rownum + " style='display:none'>" + ($(thistr.find("div[id^=lessToneType]")[0]).html())
				+ "</div><div id='lessToneTypeShow"+rownum +"'>" + ($(thistr.find("div[id^=lessToneType]")[1]).html()) + "</div>";
		
			// 风格
		var styleDiv = "<div id=style" + rownum + " style='display:none'>" + ($(thistr.find("div[id^=style]")[0]).html()) + "</div><div id='styleShow"+rownum +"'>"
				+ ($(thistr.find("div[id^=style]")[1]).html()) + "</div>";
		
		// 系列
		var seriesDiv = "<div id=series" + rownum + " style='display:none'>" + ($(thistr.find("div[id^=series]")[0]).html()) + "</div><div id='seriesShow"+rownum +"'>"
				+ ($(thistr.find("div[id^=series]")[1]).html()) + "</div>";
		...
		// + "</div>";
		"<div id=certificate" + rownum + " style='display:none'>" + ($(thistr.find("div[id^=certificate]")[0]).html()) + "</div><div id='certificateShow"+rownum +"' style='height:50px;overflow:hidden;'>"
				+ ($(thistr.find("div[id^=certificate]")[1]).html()) + "</div>";

		// 火彩颜色
		var toneFireColor = "<div id=toneFireColor" + rownum + " style='display:none'>" + ($(thistr.find("div[id^=toneFireColor]")[0]).html()) + "</div><div id='toneFireColorShow"+rownum +"'>"
				+ ($(thistr.find("div[id^=toneFireColor]")[1]).html()) + "</div>";
		// 主石规格
		var mainToneStyle = "<div id=mainToneStyle" + rownum + " style='display:none'>" + ($(thistr.find("div[id^=mainToneStyle]")[0]).html()) + "</div><div id='mainToneStyleShow"+rownum +"'>"
				+ ($(thistr.find("div[id^=mainToneStyle]")[1]).html()) + "</div>";

		var goldweight = $(thistr.find("div[id^=goldweight]")[0]).html();
		var goldweightdiv = "<div id=goldweight" + rownum + " style='display:none;'>" + goldweight + "</div>"; // 金重
		
		var mygoldweight = $(thistr.find("div[id^=mygoldweight]")[0]).html();
		var mygoldweightdiv = "<div id=mygoldweight" + rownum + ">" + mygoldweight + "</div>"; // 金重
		
		
		var goldweightlittleTotal = $(thistr.find("div[id^=goldweightlittleTotal]")[0]).html();
		var goldweightlittleTotaldiv = "<div id=goldweightlittleTotal" + rownum + ">" + goldweightlittleTotal + "</div>"; // 金重小计
		
		

		var imgpath = $(thistr.find("img[id^=pcimagesrc]")[0]).attr('src');
		var imgalt = $(thistr.find("img[id^=pcimagesrc]")[0]).attr("alt");
		var imaghref = "<a href=" + imgpath + " class='tooltip' title=" +$(thistr.find("div[id^=zhlhx]")[0]).html() + "(" + $("#ztjtj").val() + ")>";
		var pcimage =  "<div id=pcimage" + rownum + ">" + imaghref + "<img id=pcimagesrc  alt='" + imgalt + "' src=" + imgpath + " height='40' width='38'/></a></div>";

		
		var oldRetailPrice = "<div id=oldRetailPrice" + rownum + ">" +  $(thistr.find("div[id^=oldRetailPrice]")[0]).html() + "</div>"; // 旧市场价
		var oldYbzjbPrice = "<div id=oldYbzjbPrice" + rownum + ">" +  $(thistr.find("div[id^=oldYbzjbPrice]")[0]).html() + "</div>"; // 旧的
		var oldTonePrice = "<div id=oldTonePrice" + rownum + ">" + $(thistr.find("div[id^=oldTonePrice]")[0]).html() + "</div>"; // 旧石头价
		var oldTechnicsPrice = "<div id=oldTechnicsPrice" + rownum + ">" + $(thistr.find("div[id^=oldTechnicsPrice]")[0]).html() + "</div>"; // 旧工艺价
		var extwg = "<div id=extwg" + rownum + ">" + $(thistr.find("div[id^=extwg]")[0]).html() + "</div>"; // 旧extwg
		var matkl = "<div id=matkl" + rownum + ">" + $(thistr.find("div[id^=matkl]")[0]).html() + "</div>"; // 旧matkl
		var kbetr = "<div id=kbetr" + rownum + ">" + $(thistr.find("div[id^=kbetr]")[0]).html() + "</div>"; // 旧kbetr
		var oldCostPrice = "<div id=oldCostPrice" + rownum + ">" + $(thistr.find("div[id^=oldRetailPrice]")[0]).html() + "</div>"; // 旧市场价
		...
		if (orderType == 1)
			$("#totalgoldweight").text((Number(Number($("#totalgoldweight").text()) + Number(goldweight)))); // 总金重

		//$("#totalsubtotal").text((Number(Number($("#totalsubtotal").text()) + littleTotal))); // 总价格
...
//			row = row + "<td class=" + style + " align='right'>" + goodTotalWeight + "</td>"; // 总重
		...
		row = row + "<td class=" + style + ">" + selectWerks + "</td>"; // 门店
...
//			}
	...
	function submitOrder() {
		...
		$.ajax({
			...
			success : function(data) {
				...
				if(data.error != null)
					jAlert(data.error,"错误提示",function(e){});
				else if(data.success != null)
					jAlert(data.success,"提示",function(e){location.reload();});
				else
					jAlert("成功！","提示",function(e){location.reload();});	
	...
	//	}
		...
		$("#tablecontent tr").each(function() { // 得到每一个tr
			...
			_technics = $(earchobject.find("div[id^='technics']")[0]).html();// 工艺类型
			...
			_style = $(earchobject.find("div[id^='style']")[0]).text(); // 风格
			_series = $(earchobject.find("div[id^='series']")[0]).text(); // 系列
			...
			_certificate = $(earchobject.find("div[id^='certificate']")[0]).html();// 裸石证书
			...
			_selectWerks = earchobject.find("div[id^='selectWerks']").html();// 分配门店
			if(_selectWerks == '' || _selectWerks == null)
				alreadySelectWerks = false;
			...
			if (orderType == 2) {
				...
				tablestr = tablestr + "\"goodTotalWeight\":\"" + $.trim(_goodTotalWeight) + "\",";
			...
			tablestr = tablestr + "\"style\":\"" + $.trim(_style) + "\",";
			tablestr = tablestr + "\"series\":\"" + $.trim(_series) + "\",";
			tablestr = tablestr + "\"selectWerks\":\"" + $.trim(_selectWerks) + "\",";
		...
		//alert(alreadySelectWerks);
		if(alreadySelectWerks)
			return tableinfo;
		else
			return 'error0';
	}
	...
	function chosematnrinfo(item, type) {
		...
		if(($("#matnrinput").css("display") != 'none')){
			...
			if (_ordertype == "ZOR4") {
				...
				$("#charg").removeClass("inputkey");
				if (orderType == 1) {
					if($("#goldType").attr(""))$("#goldType").focus();
				} else if (orderType == 2) {
					$("#toneType").focus();
				} else {
					$("#bowlderType").focus();
	...
	function getYshckBymatnr(matnr, kondm, slbm, slxz, slys, zsjd, zgy, extwg) {
		if (orderType == 1) {
			...
			$.ajax({
				...
				dataType : 'json',
				type : 'post',
				data : "&matnr=" + matnr + "&toneType=" + slbm + "&kondm=" + kondm + "&toneColor=" + slys + "&toneNeatness=" + zsjd + "&technics=" + zgy + "&extwg=" + extwg,
				success : function(data) {
...
//				$("#submit").attr("disabled", true);
				...
				if (opmode != "view") {
					...
					$("#headremark").attr("disabled", false);
		...
		function creatTableRow(salesorderid) {
			$.getJSON("longhaul/pos/choiceorder/choiceOrderSystem.ered?reqCode=getChoiceOrderItem&postType=1", {
				...
				$.each(data, function(key, val) {
					...
					_tempcharg = val.batchnumber;
					_tempmatnr = val.materialnumber;
				
					var charg = "<div id=charg" + rownum + ">" + _tempcharg.toUpperCase() + "&nbsp;</div>"; // 批次
					var matnr = "<div id=matnr" + rownum + ">" + _tempmatnr.toUpperCase() + "&nbsp;</div>"; // 物料
				
					var zhlhx = "<div id=zhlhx" + rownum + " class='divmatnrdesc'>" + val.materialdesc + "&nbsp; </div>"; // 商品ID
...
//					else
					...
					// 总重
					var goodTotalWeight = "<div id=goodTotalWeight" + rownum + ">" + Number(val.goodtotalweight) + "</div>";
					...
					var ifNeedLessToneDiv = "<div id=ifNeedLessTone" + rownum + " style='display:none'>" + ifNeedLessTone + "</div><div id='ifNeedLessToneShow"+rownum+"'>" + (ifNeedLessTone == "X" ? "是" : "否") + "</div>"; // 是否需要副石Div
						...
						// 风格
				var styleShow = val.style != null ? (val.style.substring((val.style.indexOf('|-|') != -1 ? val.style.indexOf('|-|')+3 : 0))) : "&nbsp;";
							
				var styleDiv = "<div id=style" + rownum + " style='display:none'>" + (val.style) + "</div><div id='styleShow"+rownum +"'>"
						+ (styleShow) + "&nbsp;</div>";
				
				// 系列
				var seriesShow = val.series != null ? (val.series.substring((val.series.indexOf('|-|') != -1 ? val.series.indexOf('|-|')+3 : 0))) : "&nbsp;";
				var seriesDiv = "<div id=series" + rownum + " style='display:none'>" + (val.series) + "</div><div id='seriesShow"+rownum +"'>"
						+ (seriesShow) + "&nbsp;</div>";
					...
					var certificateDiv = "<div id=certificate" + rownum + " style='display:none'>" + (val.certificate) + "</div><div id='certificateShow"+rownum +"' style='width:80px;height:50px;overflow:hidden;'>"
							+ (certificateShow) + "&nbsp;</div>";
					...
					var imaghref = "<a href=sappic/" + imgpath + " class='tooltip' title=" + $("#zhlhx").val() + "(" + $("#ztjtj").val() + ")>";
					...
					var selectWekrsShow =  val.selectwerks != null ? ((val.selectwerks.substring((val.selectwerks.indexOf('|-|') != -1 ? val.selectwerks.indexOf('|-|')+3 : 0))) + "&nbsp;") : "&nbsp;";
					//var selectWerksShortShow = selectWekrsShow.substring(0,50) + '......'
					var selectWerks = "<div id=selectWerks" + rownum + " style='display : none;'>"+(val.selectwerks)+"</div><div id=selectWerksShow" + rownum + " style='height:50px; width:150px;overflow:hidden;'>" + selectWekrsShow + "</div>"; // 门店
...
//					}
					...
					if (orderType == 1 || orderType == 2)
						row = row + "<td class=" + style + " align='left'>" + technics + "</td>";
...
//						row = row + "<td class=" + style + " align='right'>" + goodTotalWeight + "</td>"; // 总重
					...
					row = row + "<td class=" + style + " align='left'>" + selectWerks + "</td>"; // 门店
...
//				var style = "";
//				_chargtype = val.upchoiceorderitem == "00" ? "charg" : "gift";
//				_ordertype = $("#ordertype").val(); // 订单类型字段
//				style = val.upchoiceorderitem == "00" && (_ordertype != "ZRE2" && _ordertype != "ZKL") ? "tdtoplevel" : "tdgiftlevel";
...
//				lowerlevel = val.upchoiceorderitem == null ? "" : val.upchoiceorderitem;
//				if (val.upchoiceorderitem != null) { // 最大号
//					toplevelnumber = Number(val.choiceorderitem) / 10 + 1;
//					// toplevelnumber =toplevelnumber+1;
//				}
//				toplevelstr = "<div id=leveladdgifts" + val.choiceorderitem + "  style='display:none'>" + lowerlevel + "</div>";
//				posnrnumber = "<div  id=posnrnumber" + key + "> " + val.choiceorderitem + " </div>" + "<div id=upnumber" + _chargtype + key + "  style='display:none'>" + val.salesorderitem + "</div>" + toplevelstr + "";
...
//				}
	...
	$("div[id^='toneWeight']").live('click',function(){
		...
		inputIns.keyup(function(event) {
			...
			if (keycode == 13 && inputIns.unbind("blur")) {
				if (checkNum(this)) {
					divedit.html(Number($(this).val())); // 设置新值
					getNewPriceInTable(thistr);
				} else {
					$(this).focus();
				}
...
//				});
		...
		//alert(valueDiv.text().substring(0,valueDiv.text().indexOf('|-|')));
		inputIns.unbind().keydown(function(event) {
			...
			if (keycode == 13 && inputIns.unbind("blur")) {
				...
				for(var i = 0 ; i <$(this).find("option:selected").size();i++ ){
					text = text + $($(this).find("option:selected")[i]).text()+"<br/>";
				}
...
//				});
	...
	$("div[id^='selectWerksShow']").live('click',function(){
		if(opmode == 'view')
			return;
		var divedit = $(this);
		if (divedit.children("select").length > 0) {
		...
		var inputIns = $("#werksInfo").clone(true); 
		inputIns.css("display","");
		var oldtext = $(this).html(); // 保存原有的值
		//inputIns.width(divedit.width()); // 设置INPUT与DIV宽度一致
		//inputIns.val(divedit.html()); // 将本来单元格DIV内容copy到插入的文本框INPUT中
...
//				});
		// 处理回车和ESC事件
		var thistr = $(this).parents("tr:first");
		var valueDiv = thistr.find("div[id^='selectWerks']");
		//var showDiv = thistr.find("div[id^='selectWerksShowHover']");
		...
		inputIns.unbind().keydown(function(event) {
			...
			if (keycode == 13 && inputIns.unbind("blur")) {
				...
				var text = "选款门店数量："+$(this).find("option:selected").size() +"<br />";
				for(var i = 0 ; i <$(this).find("option:selected").size();i++ ){
					text = text + $($(this).find("option:selected")[i]).text()+"<br/>";
				}
				var value = $(this).val() +'|-|' + text;
				...
				marketPrice = $(thistr).find("div[id^='retailPrice']").text();
				var number = Number($(thistr).find("div[id^='number']").text());
				var retailPrices = marketPrice.split('-');
				var selectWerks = $(thistr).find("div[id^=selectWerks]");
				var werks = $(selectWerks[0]).text();
				werks = werks.indexOf('|-|')!= -1 ? werks.substring(0,werks.indexOf('|-|')) : werks;
				var count = werks.split(',').length;
				var littleTotal = (Number(number) * Number(retailPrices[0])*count) + '-' + (Number(number) * Number(retailPrices[1])*count);// 小计金额
				$(thistr).find("div[id^='littleTotal']").text(littleTotal);
		...
		var divedit = $(this);
		var thistd = $(this).parents("td:first");
		var showDiv = $(thistd.find("div[id^='selectWerksShow']")[0]);
		...
		var show = thistd.find("div[id^='selectWerksShowHover']");
		//showDiv.hide();
		//show.show();
	}).live('mouseout',function(e){
		var divedit = $(this);
...
//	$("div[id^='selectWerksShowHover']").live('mouseout',function(e){
//		var divedit = $(this);
//		var thistd = $(this).parents("td:first");
//		var showDiv = $(thistd.find("div[id^='selectWerksShow']")[0]);
//		var show = thistd.find("div[id^='selectWerksShowHover']");
...
//	})
	...
	$("div[id^='styleShow']").live('click',function(){
		if(opmode == 'view')
			return;
		var divedit = $(this);
		if (divedit.children("input").length > 0) {
		...
		// 处理回车和ESC事件
		var thistr = $(this).parents("tr:first");
		var valueDiv = thistr.find("div[id^='style']");
		inputIns.val(valueDiv.text().substring(0,valueDiv.text().indexOf('|-|')));
		inputIns.unbind('keydown').keydown(function(event) {
			var keycode = event.which;
			if (keycode == 13 && inputIns.unbind("blur")) {
	...
	$("div[id^='seriesShow']").live('click',function(){
		if(opmode == 'view')
			return;
		var divedit = $(this);
		if (divedit.children("input").length > 0) {
		...
		var inputIns = $("#series").clone(true); 
		var oldtext = $(this).html(); // 保存原有的值
		//inputIns.width(divedit.width()); // 设置INPUT与DIV宽度一致
		//inputIns.val(divedit.html()); // 将本来单元格DIV内容copy到插入的文本框INPUT中
		divedit.html(""); // 删除原来单元格DIV内容
		...
		// 处理回车和ESC事件
		var thistr = $(this).parents("tr:first");
		var valueDiv = thistr.find("div[id^='series']");
		inputIns.val(valueDiv.text().substring(0,valueDiv.text().indexOf('|-|')));
		inputIns.unbind('keydown').keydown(function(event) {
			var keycode = event.which;
			if (keycode == 13 && inputIns.unbind("blur")) {
						...
						success : function(data) {
							if (data == null || data == "") {
								jAlert("请注意,没有维护金价!","提示",function(e){
									...
									var selectWerks = $(currentTr).find("div[id^=selectWerks]");
									var werks = $(selectWerks[0]).text();
									werks = werks.indexOf('|-|')!= -1 ? werks.substring(0,werks.indexOf('|-|')) : werks;
									var count = werks.split(',').length;
...
//								var marketPrice = Number(data[0].drjj)*goldWeight;
//								marketPrice = parseInt(marketPrice*0.01)*100+"-" + parseInt(marketPrice*0.01)*100;
...
//								var goldweightTo = Number($("#goldweightTo").val());
								...
								var marketPrice = parseInt(Number(goldWeights[0])*drjj*0.01)*100+"-" + parseInt(Number(goldWeights[1])*drjj*0.01)*100;
								...
								var littleTotal = parseInt(Number(number) * Number(goldWeights[0])*drjj*0.01)*100 + '-' + parseInt(Number(number) * Number(goldWeights[1])*drjj*0.01)*100;// 小计金额
								$(currentTr).find("div[id^='littleTotal']").text(littleTotal);
			...
			var technics = $($(currentTr).find("div[id^='technics']")[0]).text().substring(0,(($($(currentTr).find("div[id^='technics']")[0]).text().indexOf('|-|') == -1) ? ($($(currentTr).find("div[id^='technics']")[0]).text().length):($($(currentTr).find("div[id^='technics']")[0]).text().indexOf('|-|'))));
			...
			var isHavePrice = $(currentTr).find("div[id^='isHavePrice']").text();
			goldType = (goldType.indexOf("|-|") != -1) ? goldType.substring(0,goldType.indexOf("|-|")) : goldType;
			if (charg != "" || matnr != "") {
				$.ajax({
					url : 'longhaul/pos/choiceorder/choiceOrderSystem.ered?reqCode=getNewMarketPrice&option=user&postType=1',
					...
					type : "POST",
					data : 
						"&toneType=" + toneType + "&gemweight=" + gemweight +"&matnr=" + matnr+"&charg=" + charg+"&goldType=" + goldType+"&lessToneType=" +
						$.trim(lessToneType)+"&toneColor=" + toneColor + "&toneNeatness=" + toneNeatness+ "&toneCount="+toneCount,
//					"&toneType=" + $("#toneType").val() + "&gemweight=" + $("#gemweight").val() + "&toneColor=" + $("#toneColor").val() + "&toneNeatness=" + $("#toneNeatness").val() + "&technics="
//							+ $("#technics").val() + "&extwg=" + $("#extwg").val() + "&matkl=" + $("#matkl").val()+"&matnr=" + matnr+"&charg=" + charg,
					success : function(data) {
						var oldRetailPrice = Number($("#oldRetailPrice").val());
			...
			var littleTotalValue = number*Number(retailPrices[0])*count+"-"+number*Number(retailPrices[1])*count;
	...
	function getTotal(){
		...
		var selectWerks = $(tableList.find("div[id^=selectWerks]"));
		...
		for(var i = 0;i<numbers.size();i++){
			var werks = $(selectWerks[2*i]).html().substring(0,($(selectWerks[2*i]).html().indexOf('|-|')!=-1 ? $(selectWerks[2*i]).html().indexOf('|-|') : 0)).split(',');
		
			var count = werks.length;
			
			totalNumber = totalNumber + Number($(numbers[i]).text())*count;
...
//			alert(littleTotal1);
	...
	function getTotalGold(){
		...
		for(var i = 0;i<numbers.size();i++){
			...
			totalNumber = totalNumber + Number($(numbers[i]).text())*count;
			totalGoldweight = totalGoldweight + Number($(goldweight[i]).text())*count;
		}
		//alert(totalGoldweight);
		totalnumber.text(totalNumber);
...
//			"&bismt="+bismt //+ "&toneshape="+toneshape
//			+"&goodsize="+goodsize;
//		else
//			data += "toneType="+toneType+"&goldType="+goldType+"&gemweight="+gemweight+
//				"&technics="+technics+"&bismt="+bismt +"&mainToneStyle="+mainToneStyle
...
//					$("#matnrselect").append($("<option>"+ retdata[i] +"</option>"));
//				}
//			},
//			failure : function(e){
//			}
...
//	});
	...
	function getmatnrinfo(){
		...
		if(orderType=='J1' || orderType=='J2' || orderType=='J3' || orderType=='J4' || orderType=='J5')
			...
			data += "toneType="+toneType+"&goldType="+goldType+"&gemweight="+gemweight+
				"&technics="+technics+"&bismt="+bismt 
				//+"&mainToneStyle="+mainToneStyle
				+"&goodsize="+goodsize+"&ifNeedLessTone="+ifNeedLessTone;
		else 
			data += "toneType="+toneType+"&goldType="+goldType+
				"&technics="+technics+"&bismt="+bismt +"&mainToneStyle="+mainToneStyle
				+"&goodsize="+goodsize+"&ifNeedLessTone="+ifNeedLessTone;
		 $("#matnrselect").empty();
	...
	function getBismtInfo(bismt,myslxz,myslgg,mygy,myjlbm,myslbm,mystyle,myseries){
		$.ajax({
...
//						});
				...
				if(slbm.length != 0){
					...
					if($("#type").val() == "Y1"){
						...
						for(var i = 0 ; i < slbm.length; i++){
							...
							$("#bowlderType").append("<option value='"+value+"'"+(value == myslbm?'selected':'')+">"+text+"</option>");
						}
					}else{
						$("#toneType").empty();
						$("#toneType").append("<option value=''>请选择...</option>");
				...
				if(styles.length != 0){
					$("#style").empty();
					$("#style").append("<option value=''>请选择...</option>");
					
					for(var i = 0 ; i < styles.length; i++){
						var value = styles[i].zcpfg;
						var text = styles[i].tcpfg;
						$("#style").append("<option value='"+value+"'"+(value == mystyle?'selected':'')+">"+text+"</option>");
					}
				}
				...
				if(series.length != 0){
					...
					for(var i = 0 ; i < series.length; i++){
						var value = series[i].zcpxl;
						var text = series[i].tcpxl;
						$("#series").append("<option value='"+value+"'"+(value == myseries?'selected':'')+">"+text+"</option>");
					}
				}
			}
		});
...
//		}
	...
	$("#gemweight").change(function(e){
		...
		var goodSize;
		goodSizeStr = goodSizeStr.replace(/\s/g,"");
		if (goodSizeStr.indexOf("/") != -1) {
		...
		} else if (goodSizeStr.indexOf("-") != -1 ||  goodSizeStr.indexOf("－") != -1) {
			goodSize = goodSizeStr.split("-");
