		...
		$.getJSON(
				'longhaul/pos/choiceorder/choiceOrderSystem.ered?reqCode=getChoiceOrderHeader&option=user&postType=1', 
					...
					$.each(data, function(key, val) {
						...
						var _herf = basepath
								+ "longhaul/pos/choiceorder/choiceOrderSystem.ered?reqCode=getChoiceOrderByOrderId&option=user&opmode=view&postType=1&choiceOrderId="
								+ val.choiceorderid + "&WERKS=" + val.werks + "&operator=" + operator;
						row = row
				...
				success : function(data){
					if('X' == data){
						...
						window.open(basepath
								...
								+ _salesorderid + "&operator=" + operator);
					}
				}
			});
			} else {
	...
	$("#del").click(function(event) {
		...
		jConfirm('确认要删除订单吗?', '提示', function(r) {
			if (r == true) {
				...
				$.ajax({
							url : "longhaul/pos/choiceorder/choiceOrderSystem.ered?reqCode=delOrder&option=user&postType=1",
							cache : false,
							data : {
								orderhead : orderhead
							},
