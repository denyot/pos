      ...
      <table width="800px" border="1" align="center">
        <tr>
          <td colspan="4" align="center">
            <font class="titlehead"><span class="switch_type_span">维修</span>管理 </font>
          </td>
        </tr>
        <td colspan="4" align="left" width=400px>
          <font class="numberInfo"><span class="switch_type_span">维修</span>单号：${map.service_number }
            顾客姓名：${map.member_name } </font>
        </td>
        <tr>
          <td>
            到柜日期:
          ...
          <td class="inputleft">
            <input name="to_cabinet_date" id="to_cabinet_date" value="${map.to_cabinet_date }" class="inputreadonly"
        ...
        <tr>
          ...
          <td>
            <span class="not_satisfied_reason_span">不满意原因:</span>
          </td>
          <td class="inputleft">
            <span class="not_satisfied_reason_span"><input name="not_satisfied_reason" id="not_satisfied_reason"
                value="${map.not_satisfied_reason }" /> </span>
          </td>
        </tr>
        <tr>
          <td>
          ...
          <td class="inputleft">
            <input value="${map.real_repair_amount_cus }" class="inputreadonly" readonly="readonly" />
            元
          </td>
          <td>
            &nbsp;
        ...
        <tr>
          ...
          <td class="inputleft">
            <input name="real_customer_amount" id="real_customer_amount" value="${map.real_customer_amount }" />
            元
          </td>
          <td>
            顾客取货日期:
        ...
        <tr>
          ...
          <td class="inputleft">
            <select name="accept_people_third_select" id="accept_people_third_select" multiple="multiple"
              style="width: 200px">
            </select>
            <input name="accept_people_third" id="accept_people_third" type="hidden" value="${map.accept_people_third }" />
          </td>
        </tr>
        <tr>
          <td>
          ...
          <td class="inputleft">
            <textarea name="remark3" id="remark3">${map.remark3 }</textarea>
          </td>
          <td></td>
          <td class="inputleft">
          </td>
        ...
        <tr>
          <td colspan="4">
            ...
            <input type="button" id="printOrder" value="打印" class="orersystembutton" />
...
$(function () {
  ...
  if(service_type=='1'){
    $('.switch_type_span').text('维修');
  }else if(service_type=='2'){
    $('.switch_type_span').text('改款');
  }else if(service_type=='3'){
    $('.switch_type_span').text('配对');
  }
//选择满意度
  satisfiedLinkage();
  function satisfiedLinkage(){
    ...
    if(re=='满意'){
      $('.not_satisfied_reason_span').hide();
    }else if(re=='不满意'){
      $('.not_satisfied_reason_span').show();
    }
  }
...
//营业员
  var selectman=$('#accept_people_third').val();
  getsaleman(selectman,"EDIT");
  function getsaleman(noneselectedtext, mode) {
    $.getJSON("longhaul/pos/order/orderSystem.ered?reqCode=getsaleman&postType=1&random=" + Math.random(), {
      ...
      $.each(data, function(key, val) {
        ...
        noneselectedtext = mode == "ADD" ? key == 100000 ? val.assistant_name : noneselectedtext : noneselectedtext; // 默认不需要选择
        noneselectedtext = mode == "ADD" ? "请选择营业员" : noneselectedtext;
        selectedadd = mode == "ADD" ? key == 100000 ? "selected" : "" : "";
        selectededit = mode == "EDIT" ? noneselectedtext.indexOf(val.assistant_name) > -1 ? "selected" : "" : "";
  ...
  $("#repairSubmitBtn").click(function() {
     ...
     $.ajax({  
             url :'longhaul/pos/aftersales/aftersales.ered?reqCode=updateInfo&postType=1&service_type='+service_type+'&step=3', 
             type:'post',    
             dataType:'json',   
             data:params,    
...
//打印
  $("#printOrder").click(function(){
  	var ordernum = $("#service_number").val();

  	if(service_type == 1){
  		window.showModalDialog(basepath + "longhaul/pos/aftersales/wx_print.jsp?ordernum=" + ordernum , 'printWindow', 'dialogWidth=950px;dialogHeight=500px');
  	}else if(service_type == 2){
  		window.showModalDialog(basepath + "longhaul/pos/aftersales/gk_print.jsp?ordernum=" + ordernum , 'printWindow', 'dialogWidth=950px;dialogHeight=500px');
  	}else if(service_type == 3){
  		window.showModalDialog(basepath + "longhaul/pos/aftersales/pd_print.jsp?ordernum=" + ordernum , 'printWindow', 'dialogWidth=950px;dialogHeight=500px');
  	}
