		...
		$.getJSON(
				"longhaul/pos/aftersales/aftersales.ered?reqCode=getAftersalesList&postType=1&random="
					...
					$.each(data, function(key, val) {
						...
						var old_commodity_barcode = val.prepared_commodity_barcode == null? "&nbsp;": val.prepared_commodity_barcode;
						row = row + "<td>" + old_commodity_barcode + "</td>";
						var trade_name = val.faceliftname == null? "&nbsp;": val.faceliftname;
...
$("#print_facelift").click(function(event) {
	...
	if ($("input[name='useroption']:checked").size() > 1) {
		...
		var url = basepath + "longhaul/pos/aftersales/aftersales.ered?reqCode=getServiceInfoById&postType=1&service_type=2&werks=" + WERKS +"&step=detail&whichp=s&service_number="+_service_number;
	    window.open(url);
	}
});
