...
Ext.onReady(function() {
	...
	var store = new Ext.data.Store({
		// 获取数据的方式
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getMatnrList'
		}),
		// 数据读取器
		reader : new Ext.data.JsonReader({
	...
	store.on('beforeload', function() {
		this.baseParams = {
			...
			extwg : extwg_combo.getValue(),
			zslbm : tone_combo.getValue(),
			zjlbm : gold_combo.getValue(),
			maktx : Ext.getCmp("name").getValue(),
			toneweight : Ext.getCmp("toneweight").getValue()
		};
	});
	// 每页显示条数下拉选择框
	...
	var extwgInfo = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getextwgInfo'
		}),
		reader : new Ext.data.JsonReader({}, [ {
			name : 'extwg'
		}, {
	...
	extwgInfo.load({
		callback : function(records, options, success) {
			extwg_combo.setValue('PE');
		}
	}); // 如果mode : 'local',时候才需要手动load();

	var extwg_combo = new Ext.form.ComboBox({
		name : 'extwg',
		triggerAction : 'all',
		store : extwgInfo,
		emptyText : '款式',
	...
	var toneInfo = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=gettoneInfo'
		}),
		reader : new Ext.data.JsonReader({}, [ {
			name : 'zslbm'
		}, {
	...
	toneInfo.load({
		callback : function(records, options, success) {
			tone_combo.setValue('DI');
		}
	}); // 如果mode : 'local',时候才需要手动load();

	var tone_combo = new Ext.form.ComboBox({
		name : 'tonetype',
		triggerAction : 'all',
		store : toneInfo,
		emptyText : '石料',
	...
	var goldInfo = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getgoldInfo'
		}),
		reader : new Ext.data.JsonReader({}, [ {
			name : 'zjlbn'
		}, {
	...
	goldInfo.load({
		callback : function(records, options, success) {
			gold_combo.setValue('K18W');
		}
	}); // 如果mode : 'local',时候才需要手动load();

	var gold_combo = new Ext.form.ComboBox({
		triggerAction : 'all',
		store : goldInfo,
		emptyText : '金料',
		displayField : 'zjlms',
	...
	var tbar = new Ext.Toolbar({
		items : [ {
			...
			emptyText : '名字',
			id : "name"
		}, '-', kondm_combo, '-', extwg_combo, '-', tone_combo, '-', gold_combo, '-', {
			xtype : 'textfield',
			emptyText : '主石重',
			id : "toneweight"
		}, '-', {
