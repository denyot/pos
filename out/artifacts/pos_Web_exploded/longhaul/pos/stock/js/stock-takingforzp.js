...
Ext.onReady(function() {
	...
	var storeWaitToTakingInfo = new Ext.data.Store({
		// 获取数据的方式
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getStockTakingForZPInfo'
		}),
		// 数据读取器
		reader : new Ext.data.JsonReader({
			id : 'callTime'
	...
	var stockTakingIdInfo = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getStockTakingForZPId'
		}),
		reader : new Ext.data.JsonReader({}, [ {
			name : 'id'
		}, {
	...
	// 表格实例
	var gridnotaking = new Ext.grid.EditorGridPanel({
		// 表格面板标题,默认为粗体，我不喜欢粗体，这里设置样式将其格式为正常字体
		height : 300,
		frame : true,
		// autoHeight : true,
	...
	// 表格实例
	var gridtaked = new Ext.grid.EditorGridPanel({
		// 表格面板标题,默认为粗体，我不喜欢粗体，这里设置样式将其格式为正常字体
		height : 300,
		frame : true,
		// autoHeight : true,
	...
	grid.on("cellclick", function(pGrid, rowIndex, columnIndex, e) {
		...
		if (fieldName == 'submititem' && columnIndex == 3) {
			...
			var lgort = currLgort == null ? record.get('lgort') : currLgort;
			...
			// if (rowsmatnr == matnr && rowslgort == lgort) {
			Ext.Ajax.request({
				url : 'stockSystem.ered?reqCode=submitStockTakingFroZPInfo&option=user&postType=1',
				method : 'post',
				params : {
					id : currentStockTakingId,
					charg : charg,
				...
				success : function(data) {
					...
					if (retData.success != null) {
						...
						Ext.getCmp("noStockTakingCount").setText("未盘物料号：" + storeWaitToTakingInfo.data.items.length);
	...
	var chargLgortInfo = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getMatnrLgortInfoForZP'
		}),
		reader : new Ext.data.JsonReader({}, [ {
			name : 'werks'
		}, {
				...
				items : [ {
					...
					handler : function() {
						...
						if (lgort == '') {
							...
							var lgortname = Ext.get("lgortname").dom.value;
							var lgortStr = lgortname + '-' + lgort;
							rec.set("lgort",lgort);
							rec.set("lgortStr",lgortStr);
						}
					}
				} ]
			} ]
	...
	var orderStockTakingWindow = new Ext.Window({
		title : '<span class="commoncss">预约盘点日期</span>', // 窗口标题
		// layout : 'fit', // 设置窗口布局模式
		width : 600, // 窗口宽度
		iconCls : 'window_caise_listIcon', // 按钮图标
		// height : 400, // 窗口高度
			...
			if (e == 'yes') {
				...
				storeWaitToTakingInfo.load({
					...
					callback : function(records, options, success) {
						getTotal();
						getCount();
						Ext.getCmp("noStockTakingCount").setText("未盘物料号：" + records.length);
						if (success == true) {
							if (records.length == 0) {
					...
					if (retData.success != null) {
						...
						if (matnr == '') {
							...
							if (!checkRepeat('charg', charg)) {
								...
								Ext.Ajax.request({
									url : 'stockSystem.ered?reqCode=getchargforstocktaking&option=user&postType=1',
									method : 'post',
									params : {
										id : currentStockTakingId,
										charg : charg,
									...
									success : function(data) {
										...
										if (mydata[0].lgorts.length > 1) {
											Ext.Msg.alert("提示", "该物料信息有多个库位有库存，请选择目前盘点的库位信息！",function(e){
												chargLgortInfo.load({
													params : {
														matnr : matnr,
														id : currentStockTakingId
										...
										if (isNeedCheck) {
											Ext.Msg.alert("提示", "该物料库存数为0，请选择该物料所在库位信息！", function(e) {
												chargLgortInfo.load({
													params : {
														matnr : matnr,
														id : currentStockTakingId
	...
	function submitStockTakingTime() {
		...
		myMask.show();
		Ext.Ajax.request({
			url : 'stockSystem.ered?reqCode=submitStockTakingTimeForZP&option=user&postType=1',
			params : {
				date : date
			},timeout : 12000000,
			success : function(data) {
	...
	function submitStockTaking() {
		...
		Ext.Ajax.request({
			url : 'stockSystem.ered?reqCode=submitStockTakedInfoForMatnr&option=user&postType=1',
			params : {
				id : currentStockTakingId
			},timeout : 12000000,
			success : function(data) {
				...
				if (retData.success != null) {
					Ext.Msg.alert("提示成功", retData.success, function(e) {
						...
						stockTakingIdInfo.load();
						Ext.getCmp("noStockTakingCount").setText("未盘物料号：");
	...
	function delStockTaking() {
		...
		Ext.Msg.confirm("提示", "确定删除吗？", function(e) {
			if (e == 'yes') {
				...
				Ext.Ajax.request({
					url : 'stockSystem.ered?reqCode=delStockTakingInfoFroZP&option=user&postType=1',
					params : {
						id : myid
					},timeout : 12000000,
					success : function(data) {
