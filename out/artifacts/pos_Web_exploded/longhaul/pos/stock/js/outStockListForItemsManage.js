		...
		.onReady(function() {
			...
			var store = new Ext.data.Store({
				...
				proxy : new Ext.data.HttpProxy({
					url : 'stockSystem.ered?reqCode=getOutStockHeaderForItemManage'
				}),
				// 数据读取器
			...
			var tbar = new Ext.Toolbar({
				items : [ {
					...
					handler : function() {
						exportExcel('stockSystem.ered?reqCode=exportOutStockHeaderForItemManageExcel');
					}
				} ]
			});
			...
			function getCheckboxValues() {
				...
				Ext.Msg.confirm("提示", "确定吗？", function(btn, text) {
					if (btn == "yes") {
					      ...
					      myMask.show();
						Ext.Ajax.request({
							url : 'stockSystem.ered?reqCode=submitOutStockItemManage',
							method : 'post',
							params : {
			...
			function getTotal() {
				...
				for ( var i = 0; i < rows1.length; i++) {
					...
					total = total + Number(rows1[i].get("tagprice") * rows1[i].get("count"));
			...
			function getCount() {
				...
				for ( var i = 0; i < rows1.length; i++) {
					count = count + Number(rows1[i].get("count"));
					total = total + Number(rows1[i].get("tagprice"));
