...
Ext.onReady(function() {
	...
	var store = new Ext.data.Store({
		// 获取数据的方式
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getReceiveGoodsHead'
		}),
		// 数据读取器
		reader : new Ext.data.JsonReader({
	...
	// sm,
	{
		...
		renderer : function(v, metadata, record, rowIndex, columnIndex, store) {
			...
			// target='_blank' >"+v+"</a>"
			metadata.attr = ' ext:qtip="<img src=\'../../../sappic/' + v + '\' onError=\'this.src=\'./images/sample1.gif\'\'/>"';
	...
	grid.on("cellclick", function(pGrid, rowIndex, columnIndex, e) {
		...
		if (fieldName == 'showdetail' && columnIndex == 1) {
			...
			// 到此你就可以继续做其他任何事情了
			// Ext.MessageBox.alert('提示', assistant_name);
			var EBELN = record.get('EBELN');
			var VBELN = record.get('VBELN');
			var DATUM = record.get('DATUM');
	...
	var selectLgort = new Ext.form.ComboBox({
		id : 'selectLgort',
		name : 'selectLgort',
		fieldLabel : '选择入库库位',
        xtype : 'combo',
	...
	function search() {
		...
		Ext.Msg.alert('提示', "<b style='color:red;font-size:18px;'>批次不存在列表中！</b>", function(e) {
			Ext.getCmp("searchText").focus(true, true);
		});
	}
	...
	function getCheckboxValues() {
		...
		// var choiceorderid = "";
		var needvalified = "";
		// var headReason = diffInfoForHead.getValue() == null ? "" :
		// diffInfoForHead.getValue();
		var postno = Ext.getCmp("postno").getValue();
		var headexplain = Ext.getCmp("headexplain").getValue();
		...
		var selectLgort = Ext.getCmp("selectLgort").getValue();
		var lgortAll ;
		if (goodType == "A" || goodType == "货品" || goodType == "J") {
			if (selectLgort == "0017") {
				lgortAll = '0017';
			} else {
				lgortAll = '0001';
			}
		} else if (goodType == "G" || goodType == "赠品") {
			lgortAll = '0014';
		} else if (goodType == "H" || goodType == "包材") {
			lgortAll = '0012';
		}
		...
		for ( var i = 0; i < rows.length; i++) {
			...
			chargitem = chargitem + "\"menge\":\"" + rows[i].get("MENGE") + "\",";

			chargitem = chargitem + "\"lgort\":\"" + ((rows[i].get("itemLgort") == null) ? lgortAll : (rows[i].get("itemLgort")).substring(0, 4))
					+ "\",";

			chargitem = chargitem + "\"ebelp\":\"" + rows[i].get("EBELP") + "\",";
			chargitem = chargitem + "\"meins\":\"" + rows[i].get("meins") + "\",";
			chargitem = chargitem + "\"choiceorderid\":\"" + rows[i].get("ZNUM") + "\",";
			chargitem = chargitem + "\"vkorg\":\"" + rows[i].get("VKORG") + "\",";
			chargitem = chargitem + "\"vtweg\":\"" + rows[i].get("VTWEG") + "\",";

			chargitem = chargitem + "\"datab\":\"" + rows[i].get("DATAB") + "\",";
			chargitem = chargitem + "\"datbi\":\"" + rows[i].get("DATBI") + "\",";
			chargitem = chargitem + "\"kbstat\":\"" + rows[i].get("KBSTAT") + "\",";
			chargitem = chargitem + "\"kbetr\":\"" + rows[i].get("bqj") + "\",";

			chargitem = chargitem + "\"lifnr\":\"" + rows[i].get("LIFNR") + "\",";

			if (rows[i].get("LIFNR") == null || rows[i].get("LIFNR") == '') {
				ifLifnr = 1;
			}
			// alert(rows[i].get("LIFNR"));
			var reason = "";
			...
			chargitem = chargitem + "\"myexplain\":\"" + (rows[i].get("myexplain") == null ? "" : rows[i].get("myexplain")) + "\",";
			chargitem = chargitem + "\"manage\":\"" + (rows[i].get("manage") == null ? "" : rows[i].get("manage")) + "\"}";
			...
			// lgort += rows[i].get("itemLgort") == null ? lgortAll + ',' :
			// rows[i].get("itemLgort") + ',';
			// ebelp += rows[i].get("EBELP") + ",";
			//			

			if (rows[i].get('reason') != null && rows[i].get('reason') != '') {
				needvalified = '1';
			}

			// reason += (rows[i].get('reason') != null ? rows[i].get('reason')
			// : "") + '&';
			// myexplain += (rows[i].get('myexplain') != null ?
			// rows[i].get('myexplain') : "") + '&';
				...
				Ext.Ajax.request({
					...
					success : function(data) {
						...
						if (retData.success != null) {
							...
							myMsg += (retData.success == null ? '' : (retData.success + "<br />")) ;
							
							if (goodType == "A") {
//								Ext.Msg.confirm("提示成功,是否更新价格状态？", retData.success, function(e) {
//									if (e == 'yes') {
										Ext.Ajax.request({
...
//								});
								...
								Ext.Msg.alert("提示成功", myMsg, function(e) {
									myMask.hide();
									thirdWindow.hide();
									queryCatalogItem();
								});
	...
	function getCount() {
		...
		Ext.getCmp("totalmoney2").setText(total.toFixed(3));
