	...
	Ext.data.Connection.prototype.timeout='300000';
			...
			// 列宽
				...
				renderer : function(v, metadata, record, rowIndex, columnIndex, store){
					metadata.attr = ' ext:qtip="' + (v==null?'':v) + '"';    
					return v;
				}
			}, {
				header : '整单差异处理',
			...
			var tbar = new Ext.Toolbar({
				items : [ {
					...
					handler : function() {
						exportExcel('stockSystem.ered?reqCode=exportOutStockHeaderExcel');
					}
				} ]
			});
...
//			}
		...
		Ext.Msg.confirm("提示", "确定提交吗？", function(btn, text) {
			if (btn == "yes") {
			      ...
			      myMask.show();
				Ext.Ajax.request({
					url : 'stockSystem.ered?reqCode=submitOutStockHeadManage',
					method : 'post',
					params : {
						chargitem : chargitem
					},timeout : 12000000,
