$(function() {
	...
	function getchargbyuser() {
		...
		$.ajax({
			url : "longhaul/pos/stock/stockSystem.ered?reqCode=getpcxx&option=user&postType=1&werks="
			...
			success : function(data) {
				...
				$.map(data, function(item) {
							...
							var goldweight = item.zclzl == null
									? 0
									: item.zclzl; // 金重
							...
							var charglabst = item.labst == null
									? 1
									: item.labst; // 批次数量
	...
	$("#number").keydown(function(event) {
		...
		if (event.keyCode == 13) {
			...
			if ($('#charg').val() == ""
					|| $("#showmatnrinfo").css("display") == 'none') {
				alert("请先录入商品信息!");
				$("#charg").select();
				return;
			}
	...
	function btnAddRow() {
		...
		var charg = "<div id=charg" + rownum + ">" + _tempcharg.toUpperCase()
				+ "&nbsp;</div>"; // 批次
		...
		var uniteprice = "<div id=uniteprice" + rownum + ">"
				+ Number($("#ztjtj").val()).toFixed(2) + "&nbsp;</div>"; // 单价
				var realcount = Number($("#goodtype").val() != '8' ? 1 : Number(_number));
		var numberShow = "<div id=Shownumber" + rownum + ">"
				+ (realcount)
				+ "</div>"; // 数量
		var number = "<div id=number" + rownum + " style='display:none;'>"
				+ Number(_number) + "</div>"; // 数量
		// var personcost = "<div id=personcost" + rownum + ">" +
		// Number($("#personcost").val()).toFixed(2) + "</div>"; // 人工费用
		var goldweight = "<div id=goldweight" + rownum + ">"
				+ Number($("#goldweight").val()).toFixed(3) + "</div>"; // 金种
		var goldvalue = "<div id=goldvalue" + rownum + " style='display:none'>"
				+ $("#goldvalue").val() + "</div>"; // 金种
		...
		var discount = "<div id=discount" + rownum + ">" + $("#discount").val()
				+ "</div>"; // 折扣
		...
		$("#totalprice")
				.text((Number($("#totalprice").text()) + Number(_ztjtj))
						.toFixed(2)); // 总的标签价格
		$("#totalnumber")
				.text((Number($("#totalnumber").text()) + Number(realcount))
						.toFixed(0)); // 总数量
		...
		row = row + "<td class=" + style + " align='right'>" + numberShow
				+ number + "</td>";
	...
	$('#delrow').live('click', function() {
		...
		do { // 删除 赠品结点改变相应以下赠品结点序号
			...
			nextupnumberlevel = nexttrobj.find("div[id^='upnumberlevel']")
					.text(); // 下个Tr上层
	...
	//if (opmode != "edit") {
		$.ajax({
			url : "longhaul/pos/stock/stockSystem.ered?reqCode=getWerks&option=user&postType=1&werks="
					+ "01DL" + "&random=" + Math.random() + "",
			dataType : "json",
			async : false,
			// data : "charg=" + $.trim(_charg) + "&chargtype=" + _chargtype,
...
//				});
	...
	$("#submit").click(function() {
		if ($("#ordertype").val() == "ZOR4" && !checkChangeInfo()) {// 检查换货输入信息
			...
			trlength = $("#tablecontent tr").length;
			if (trlength == 2) {
				jAlert("没有录入货物,不能提交!", "提示", function(e) {
						});
				return false;
			} else {
				if ($("#inwerk").val() == "") {
					jAlert("请选择收货门店!", "提示", function(e) {
								$("#inwerk").focus();
							});
					return false;
				}
	...
	function getordertableinfo() {
		...
		var _inwerk = $("#inwerk").data('mywerks');
		...
		var _totalcount = $("#totalnumber").text();
		orderhead = orderhead + "\"totalcount\":\"" + $.trim(_totalcount) + "\",";

		var _totalgoldweight = $("#totalgoldweight").text();
		...
		$("#tablecontent tr").each(function() { // 得到每一个tr
					...
					// _uniteprice =
					// earchobject.find("div[id^='uniteprice']").text();
					...
					_goldweight = earchobject.find("div[id^='goldweight']")
							.text();
					// _goldvalue =
					// earchobject.find("div[id^='goldvalue']").text();
					// _receivable =
					// earchobject.find("div[id^='receivable']").text();
					_ztjtj = earchobject.find("div[id^='ztjtj']").text();
					_realprice = earchobject.find("div[id^='realprice']")
							.text();
					// _discount =
					// earchobject.find("div[id^='discount']").text();
					// _ordertItemype =
					...
					// $.trim(_realprice) + ",";
					// tablestr = tablestr + "\"storediscount\":" +
					// $.trim(_discount) + ",";
	...
	function creatTablehead(salesorderid) {
		...
		$
				.getJSON(
						"longhaul/pos/stock/stockSystem.ered?reqCode=getOrderhead&postType=1",
						{
							outid : salesorderid
						}, function(data) {
							...
							$.each(data, function(key, val) {
								...
								$("#inwerk").data("mywerks", val.inwerks);
								
								for(var i = 0 ; i <options.length;i++ ){
									if(options[i].werks == val.inwerks){
														$("#inwerk").val(options[i].name1);
													}
								}
...
//										
//										$.map(data, function(item) {
//												if(item.werks = val.werks){
//													$("#inwerk").val(item.name1);
//												}
//											
////													$("#inwerk")
////															.append("<option value='"
...
//								});
	...
	function creatTableRow(salesorderid) {
		$
				.getJSON(
						"longhaul/pos/stock/stockSystem.ered?reqCode=getOrderItem&postType=1",
						{
							outid : salesorderid,
							random : Math.random()
						}, function(data) {
							...
							$.each(data, function(key, val) {
								...
								toplevelstr = "<div id=leveladdgifts"
										+ upnumber + " style='display:none'>"
										+ upnumber + "</div>"; // 增加礼品时记录礼品行号包括有多少礼品
								var ordertItemype = "<div id=ordertItemype"
										+ rownum + "></div>";
								var logrt = "<div id=logrt" + rownum + ">"
										+ val.outstock + "</div>";
								_total = Number($("#realprice").val());
								_ztjtj = val.bqj; // 标签价格
								_number = val.goodscount; // 数量
								ordertItemypestr = "";
								ordertItemype = "<div id=ordertItemype"
										+ rownum + " style='display:none'>"
										+ ordertItemypestr + "</div>";
								var delimage = "<img alt='删除' src='resource/image/ext/delete.png' id='delrow'  align='middle' style='cursor:hand;'>";
								var row = "<div id=" + rownum + ">" + delimage
										+ "</div>";
								var posnrnumber = "<div  id=posnrnumber"
										+ rownum + "> " + upnumber + " </div>"
										+ "<div id=upnumber" + chargtype
										+ rownum + " style='display:none'>"
										+ upnumber + "</div>" + toplevelstr
										+ "";
								var upumber = "<div id=upnumberlevel" + rownum
										+ ">" + upnumbertext + "</div>";
								...
								var charg = "<div id=charg" + rownum + ">"
										+ _tempcharg + "&nbsp;</div>"; // 批次
								var matnr = "<div id=matnr" + rownum + ">"
										+ _tempmatnr + "&nbsp; </div>"; // 物料

								var zhlhx = "<div id=zhlhx" + rownum
										+ " class='divmatnrdesc'>" + val.maktx
										+ "&nbsp; </div>";
								var number = "<div id=number" + rownum + ">"
										+ Number(_number) + "</div>"; // 数量
								var goldweight = "<div id=goldweight" + rownum
										+ ">" + (val.zclzl!=null?val.zclzl:'0' )+ "</div>"; // 金种
								var ztjtj = "<div id=ztjtj" + rownum + ">"
										+ (_ztjtj!=null? _ztjtj : '0')+ "</div>"; // 标签价格

								var imgpath = val.zmatnrt;
								var imgalt = val.zmatnrt;
								var imaghref = "<a href=" + imgpath
										+ " class='tooltip' title=" + val.matkx
										+ "(" + val.matkx + ")>";
								...
								row = row + "<td class=" + style
										...
										+ "</td>";
								row = row + "<td class=" + style + ">" + charg
										+ "</td>";
								row = row + "<td class=" + style + ">" + matnr
										+ "</td>";
								row = row + "<td class=" + style + ">" + zhlhx
										+ "</td>";
								row = row + "<td class=" + style
										+ " align='center'>" + pcimage
										+ "</td>";
								row = row + "<td class=" + style
										+ " align='right'>" + number + "</td>";
								row = row + "<td class=" + style
										+ " align='right'>" + goldweight
										+ "</td>";
								row = row + "<td class=" + style
										+ " align='right'>" + ztjtj + "</td>";
								row = row + "<td class=" + style
										+ " id='logrttd" + rownum + "' >"
										+ logrt + "</td>";
								row = row + "</tr>";
	...
	$("#inwerk").click(function(e) {
		...
		$("#dialog").dialog("open");
			...
			// if(e.keyCode != 8){
			// this.options[0].selected = "select";
			// if(this.options[0].text == '请选择'){
			// this.options[0].text = '';
			// }
