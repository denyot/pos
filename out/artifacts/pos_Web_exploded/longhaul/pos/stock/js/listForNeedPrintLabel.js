...
Ext.onReady(function() {
	...
	var store = new Ext.data.Store({
		...
		proxy : new Ext.data.HttpProxy({
			url : 'stockSystem.ered?reqCode=getOutStockHeaderForNeedPrintLabel'
		}),
		// 数据读取器
	...
	function cancleStock() {
		var needprintlabel = reasonCombo.getValue();
		var rejreason = Ext.getCmp("rejreason").getValue();
		
		
		if(needprintlabel == null || needprintlabel == ''){
			Ext.Msg.alert("提示","请先选择状态!",function(e){
			});
			return;
		}
		...
		Ext.Msg.confirm("提示", "确定提交吗？", function(btn, text) {
			if (btn == "yes") {
			      ...
			      myMask.show();
				Ext.Ajax.request({
					url : 'stockSystem.ered?reqCode=submitPrintLabel',
					method : 'post',
					params : {
						outid : currentId,
